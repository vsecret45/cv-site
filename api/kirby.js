...
const detectCvLanguageLevel = (value = '', documentLanguage = 'fr') => {
    ...
    const patterns = [
        ...
        [/fluent|courant(?:e)?|couramment/, 'fluent'],
...
const getCvLanguagesFromText = (value = '', documentLanguage = '', { requireContext = false } = {}) => {
    const source = normalize(value);
    if (!source) return [];
    ...
    const knownLanguages = [
        ...
        { fr: 'Russe', en: 'Russian', aliases: ['russe', 'russian'] },
        { fr: 'Turc', en: 'Turkish', aliases: ['turc', 'turque', 'turkish'] },
    ];
    const lines = source.split(/\r?\n/)
        ...
        .filter(Boolean);
    const detected = [];
    const languageDeclarationPattern = /\b(?:langue|langues|language|languages|je parle|nous parlons|i speak|we speak|niveau|level|maternelle|bilingue|courant(?:e)?|couramment|intermediaire|debutant(?:e)?|notions?|native|bilingual|fluent|intermediate|beginner|basic)\b/;
    const languageLevelPattern = /\b(?:niveau\s+)?[abc][12]\b|\b(?:langue maternelle|maternelle|native speaker|native|bilingual|bilingue|fluent|courant(?:e)?|couramment|professional working proficiency|working proficiency|niveau professionnel|professionnel(?:le)?|intermediate|niveau intermediaire|intermediaire|elementary level|elementary|bases solides|beginner|debutant(?:e)?|basic english|basic knowledge|basic proficiency|basic|bases?|notions?)\b/g;
    ...
    knownLanguages.forEach((languageDefinition) => {
        ...
        const line = lines.find((item) => {
            ...
            if (!languagePattern.test(normalizedItem)) return false;
            languagePattern.lastIndex = 0;
            return !requireContext
                || languageDeclarationPattern.test(normalizedItem)
                || detectCvLanguageLevel(normalizedItem, outputLanguage);
        });
        if (!line) return;

        const normalizedLine = stripAccents(line).toLowerCase();
        const match = languagePattern.exec(normalizedLine);
        const languageMentions = knownLanguages.flatMap((candidate) => {
            const candidatePattern = new RegExp(`\\b(?:${candidate.aliases.join('|')})\\b`, 'ig');
            return [...normalizedLine.matchAll(candidatePattern)].map((candidateMatch) => ({
                start: candidateMatch.index,
                end: candidateMatch.index + candidateMatch[0].length,
            }));
        }).sort((left, right) => left.start - right.start);
        const currentMentionIndex = languageMentions.findIndex((mention) =>
            mention.start === match?.index && mention.end === (match?.index || 0) + (match?.[0]?.length || 0)
        );
        const previousBoundary = currentMentionIndex > 0 ? languageMentions[currentMentionIndex - 1].end : 0;
        const nextBoundary = currentMentionIndex >= 0 && currentMentionIndex < languageMentions.length - 1
            ? languageMentions[currentMentionIndex + 1].start
            : normalizedLine.length;
        const levelCandidates = [...normalizedLine.matchAll(languageLevelPattern)]
            .filter((candidate) => candidate.index >= previousBoundary && candidate.index < nextBoundary)
            .map((candidate) => {
                const candidateEnd = candidate.index + candidate[0].length;
                const beforeLanguage = candidateEnd <= match.index;
                const afterLanguage = candidate.index >= match.index + match[0].length;
                const between = beforeLanguage
                    ? normalizedLine.slice(candidateEnd, match.index)
                    : afterLanguage
                        ? normalizedLine.slice(match.index + match[0].length, candidate.index)
                        : '';
                const isRelated = beforeLanguage
                    ? /^\s*(?:(?:en|in)\s*)?$/.test(between)
                    : afterLanguage
                        ? !/[;/]|\b(?:et|and|mais|but)\b/.test(between)
                        : true;
                const distance = beforeLanguage
                    ? match.index - candidateEnd
                    : afterLanguage
    ...
    lines.forEach((line) => {
        ...
        if (/^(?:langue|langues|language|languages)$/.test(normalizedLanguage)) return;
...
const normalizeNarratedCvSourceText = (value = '') => normalize(value)
    .replace(/\\@/g, '@')
    .replace(/([A-Za-zÀ-ÿ0-9][.!?])(?=[A-ZÀ-ÖØ-Ý](?:[A-Za-zÀ-ÿ]|[’']))/g, '$1 ');

const getNarratedCvSegmentRecords = (value = '') => normalizeNarratedCvSourceText(value)
    .split(/\n\s*\n+/)
...
const hasNarratedCvOngoingSignal = (value = '') => {
    const source = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    return /\b(?:aujourd hui|a ce jour|jusqu a present|actuellement|toujours en poste|encore en poste|j y travaille encore|je travaille encore|present|current|now|to date|still work(?:ing)?|currently work(?:ing)?)\b/.test(source)
        || /\b(?:poste|emploi|contrat|mission professionnelle|activite professionnelle) en cours\b/.test(source)
        || /\ben cours depuis\b[\s\S]{0,80}\b(?:19|20)\d{2}\b/.test(source)
...
const isNarratedCvTechnicalAside = (value = '') => {
    const source = stripAccents(normalizeText(value).toLowerCase())
        .replace(/[’']/g, ' ')
        .replace(/[_-]+/g, ' ');
    const mentionsCredential = /\b(?:tokens?|jetons?|api key|cle api|github token|tokengithub)\b/.test(source);
    const mentionsConfiguration = /(?:\.env\b|\bfichier env\b|\bvariable(?:s)? d environnement\b|\benvironment variables?\b|\bgithub\b)/.test(source);
    const troubleshootingContext = /\b(?:pourtant|toujours bon|ne (?:marche|fonctionne) pas|probleme|erreur|debug|depannage|verifie|regarde|still (?:does not|doesn t) work|check|debugging|troubleshoot)\b/.test(source);
    return mentionsCredential && mentionsConfiguration && troubleshootingContext;
};
...
const isNarratedCvUndatedWorkSegment = (value = '') => {
    const source = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    if (/\b(?:19|20)\d{2}\b|\b\d{1,2}[/.]\d{1,2}[/.](?:19|20)\d{2}\b/.test(source)) return false;
    const explicitWorkSignal = /\b(?:j ai travaille|je travaillais|j ai exerce|j exercais|j ai occupe|j occupais|salariee?|employeee?|employe(?:e)?|i worked|i was working|i have worked|i ve worked|employed)\b/.test(source);
    const copularWorkSignal = /\b(?:je suis|j etais|i was|i am)\b/.test(source)
        && /\b(?:en tant que|comme|chez|au sein de|cdi|cdd|interim|alternance|apprenti(?:e)?|stage|stagiaire|freelance|as (?:an? )?|at|job|role|contract|apprentice|internship|intern|freelance)\b/.test(source);
    const nonWorkState = /\b(?:diplomee?|titulaire|certifiee?|etudiant(?:e)?|en formation|scolarise|domiciliee?|basee?|situee?|disponible|sans emploi|diploma|graduated|degree|certified|student|studying|training|education|located|based|living|available|unemployed)\b/.test(source);
    return (explicitWorkSignal || copularWorkSignal)
        && !nonWorkState
        && !isNarratedCvGapSegment(value)
        && !isNarratedCvEditorialSegment(value);
};

const isNarratedCvStandaloneWorkPeriod = (value = '') => {
    const source = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    const hasDate = /\b(?:19|20)\d{2}\b/.test(source);
    const periodSignal = /\b(?:la periode|periode|allait de|s etendait de|du .{0,40} au|de .{0,40} a|entre .{0,40} et|from .{0,40} to|between .{0,40} and|since|depuis)\b/.test(source);
    const educationSignal = /\b(?:formation|diplome|certificat|certification|cap|bts|bac|licence|master|ecole|cfa|universite|training|degree|certificate|school|university|college)\b/.test(source);
    return hasDate && periodSignal && !educationSignal && !isNarratedCvGapSegment(value) && !isNarratedCvEditorialSegment(value);
};

const getNarratedCvWorkRecords = (records = []) => {
    ...
    records
        ...
        .forEach((periodRecord) => {
            const workRecord = records
                .filter((candidate) => candidate.paragraphIndex === periodRecord.paragraphIndex)
                .filter((candidate) => candidate.index < periodRecord.index && candidate.index >= periodRecord.index - 2)
                .filter((candidate) => !directWorkIndexes.has(candidate.index) && isNarratedCvUndatedWorkSegment(candidate.text))
                .sort((left, right) => left.index - right.index)[0];
            if (workRecord && !syntheticPeriodByWorkIndex.has(workRecord.index)) {
                syntheticPeriodByWorkIndex.set(workRecord.index, periodRecord);
            }
        });
...
const NARRATED_CV_MONTH_ALIASES = [
    ['january', /\b(?:janv?(?:ier)?|january)\b/g],
    ['february', /\b(?:fevr?(?:ier)?|february)\b/g],
    ['march', /\b(?:mars|march)\b/g],
    ['april', /\b(?:avr(?:il)?|april)\b/g],
    ['may', /\b(?:mai|may)\b/g],
    ['june', /\b(?:juin|june)\b/g],
    ['july', /\b(?:juil(?:let)?|july)\b/g],
    ['august', /\b(?:aout|august)\b/g],
    ['september', /\b(?:sept?(?:embre)?|september)\b/g],
    ['october', /\b(?:oct(?:obre)?|october)\b/g],
    ['november', /\b(?:nov(?:embre)?|november)\b/g],
    ['december', /\b(?:dec(?:embre)?|december)\b/g],
];

const getNarratedCvOrderedMonthDates = (value = '') => {
    ...
    for (const match of source.matchAll(namedMonthPattern)) {
        const monthIndex = NARRATED_CV_MONTH_ALIASES.findIndex(([, pattern]) => {
            pattern.lastIndex = 0;
            const matches = pattern.test(match[0]);
            pattern.lastIndex = 0;
            return matches;
        });
        const originalMonthText = rawSource.slice(match.index, match.index + match[0].length);
        const matchedMonthText = source.slice(match.index, match.index + match[0].length);
        const ambiguousEnglishMay = /^may\b/.test(matchedMonthText)
            && !match[1]
            && !/^May\b/.test(originalMonthText);
        if (monthIndex >= 0 && !ambiguousEnglishMay) {
            dates.push({ index: match.index, end: match.index + match[0].length, month: monthIndex + 1, year: match[1] || '' });
        }
    }
    const sortedDates = dates.sort((left, right) => left.index - right.index);
    return sortedDates.map((date, index) => {
        if (date.year) return `${date.year}-${String(date.month).padStart(2, '0')}`;
        const datedNeighbor = sortedDates
            .map((candidate, candidateIndex) => ({
                ...candidate,
                distance: Math.abs(candidate.index - date.index),
                orderDistance: Math.abs(candidateIndex - index),
            }))
            .filter((candidate) => candidate.year
                && candidate.index > date.index
                && candidate.distance <= 48
                && candidate.orderDistance <= 1)
            .sort((left, right) => left.distance - right.distance)[0];
        if (!datedNeighbor) return '';
        const between = date.index < datedNeighbor.index
            ? source.slice(date.end, datedNeighbor.index)
            : source.slice(datedNeighbor.end, date.index);
        const linkedAsRange = /^\s*(?:[–—-]|a|au|et|jusqu a|to|and|until|through)\s*$/.test(between);
        return linkedAsRange ? `${datedNeighbor.year}-${String(date.month).padStart(2, '0')}` : '';
    }).filter(Boolean);
};
...
const getNarratedCvNumericFacts = (value = '') => {
    ...
    const numberWords = withoutDates.match(/\b(?:deux|trois|quatre|cinq|six|sept|huit|neuf|dix|onze|douze|treize|quatorze|quinze|seize|vingt|trente|quarante|cinquante|soixante|cent|mille|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|twenty|thirty|forty|fifty|sixty|hundred|thousand)\b/g) || [];
    const canonicalNumberWords = {
    ...
    return new Set([...digits, ...numberWords.map((number) => canonicalNumberWords[number] || number)]);
};
...
const getNarratedCvOutputClauses = (value = '') => normalize(value)
    .split(/\s*•\s*|\r?\n+|(?<=[.!?])\s+(?=[A-ZÀ-ÖØ-Ý0-9])/)
    .map((clause) => clause.trim())
    .filter(Boolean);

const isNarratedCvSensitiveClaimNegated = (value = '', matchIndex = 0, matchLength = 0) => {
    const prefix = value.slice(Math.max(0, matchIndex - 72), matchIndex);
    const localPrefix = prefix.split(/[.;!?]|\b(?:mais|but|however)\b/).pop()?.trim() || '';
    const predicateNegationPattern = /\b(?:sans(?:\s+(?:pour\s+autant|vraiment|r[eé]ellement|n[eé]cessairement))?\s+etre|loin\s+(?:d\s+etre|de\s+(?:(?:me|te|se)\s+)?considerer|de\s+devenir)|a\s+l\s+oppose\s+de|a\s+l\s+inverse\s+de|nullement|aucunement|guere|peu|difficilement|rarement|far\s+from(?:\s+(?:being|what\s+(?:one|you)\s+would\s+call))?|nowhere\s+near|(?:the\s+)?opposite\s+of|anything\s+(?:but|other\s+than)|(?:the\s+)?least|hardly|scarcely|barely|seldom|by\s+no\s+means)(?:\s+(?:une?|la|le|les|des|an?|the|vraiment|really|genuinely|personne|person|quelqu(?:e|un|une)))?(?:\s+\w+){0,3}\s*$/;
    const predicateNegation = predicateNegationPattern.test(localPrefix)
        || predicateNegationPattern.test(prefix.trim());
    if (/(?:\b(?:ne|n)\b[\s\S]{0,40}\b(?:pas|jamais|ni|plus|guere)\b|\b(?:pas|jamais|aucun(?:e)?|ni|non|not|never|no)\b)(?:\s+\w+){0,5}\s*$/.test(localPrefix)
        || /\b(?:sans|without)\s+(?:(?:aucun(?:e)?|any|la|de|d|the)\s+)?(?:(?:maitrise|connaissance|experience|competence|savoir|utiliser|use|knowledge|experience|skill)\s+(?:de|d|en|of|with)?\s*)?$/.test(localPrefix)) {
        return true;
    }
    if (predicateNegation || /\banything\s+but\s*$/.test(prefix.trim())) return true;
    const suffix = value.slice(matchIndex + matchLength, matchIndex + matchLength + 72);
    if (/^\s*,?\s*(?:mais|but)\s+(?:plus\s+maintenant|plus\s+du\s+tout|pas\s+du\s+tout|not\s+anymore|no\s+longer|not\s+at\s+all|ce\s+n\s+est\s+(?:desormais\s+)?plus\s+(?:le\s+cas|vrai)|that(?:\s+s|\s+is)\s+no\s+longer\s+true)\b/.test(suffix)) {
        return true;
    }
    const localSuffix = suffix.split(/[.;!?]|\b(?:mais|but|however)\b/)[0]?.trim() || '';
    const postClaimNegation = /^(?:(?:\w+)\s+){0,4}(?:pas|jamais|plus|guere|non|aucun(?:e)?|zero|ni|no one|nobody)\b/.test(localSuffix);
    const precedingFrenchNegation = /\b(?:ne|n)\b(?:\s+\w+){0,5}\s*$/.test(localPrefix);
    const nobodyAfterNegatedClaim = precedingFrenchNegation
        && /^(?:(?:\w+)\s+){0,4}de personne(?:s)?\b/.test(localSuffix);
    return postClaimNegation || nobodyAfterNegatedClaim;
};

const hasNarratedCvAffirmedSensitiveClaim = (value = '', pattern) => {
    const flags = `${pattern.flags.replace(/g/g, '')}g`;
    const matcher = new RegExp(pattern.source, flags);
    return [...value.matchAll(matcher)].some((match) =>
        !isNarratedCvSensitiveClaimNegated(value, match.index, match[0].length)
    );
};

const hasNarratedCvSensitiveClaimGrounding = (value = '', sourceValue = '') => {
    const output = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    const source = stripAccents(normalizeText(sourceValue).toLowerCase()).replace(/[’']/g, ' ');
    const expertisePattern = /\b(?:expert(?:e|s|es)?|expertise|specialiste|specialisee?|senior)\b/;
    const bilingualPattern = /\b(?:bilingue|bilingual)\b/;
    const managerialOutputPattern = /\b(?:management|manager|encadrement|supervision|pilotage|leadership|managed|supervised|led)\b/;
    const teamLeadershipPattern = /(?:\b(?:gestion|encadr\w*|direction|dirig\w*|coach\w*|coaching|animation|anim\w*|formation|form\w*)\b[\s\S]{0,32}\b(?:equipes?|personnes?|staff|teams?|people)\b|\b(?:equipes?|personnes?|staff|teams?|people)\b[\s\S]{0,32}\b(?:gestion|encadr\w*|direction|dirig\w*|coach\w*|coaching|animation|anim\w*|formation|form\w*)\b)/;
    const teamOrganizationPattern = /\b(?:organisation|organization)\s+(?:de|d|des|of)\s+(?:(?:l|une?|mon|ma|mes|notre|nos|the|an?|my|our)\s+)?(?:equipes?|personnes?|staff|teams?|people)\b/;
    const managerialSourcePattern = /\b(?:management|manager|encadr\w*|supervis\w*|pilot\w*|dirig\w*|leadership|managed|supervised|led)\b/;
    const operationalResponsibilityOutputPattern = /\b(?:responsable|coordination|coordinateur|coordinatrice)\b/;
    const operationalResponsibilitySourcePattern = /\b(?:responsable|coordination|coordin\w*)\b/;
    const managerialResponsibilityPattern = /(?:\b(?:responsable|coordination|coordin\w*)\b[\s\S]{0,40}\b(?:equipe|personnes?|staff|team|people)\b|\b(?:equipe|personnes?|staff|team|people)\b[\s\S]{0,40}\b(?:responsable|coordination|coordin\w*)\b)/;
    const distinctionPattern = /\b(?:mention\s+(?:tres bien|bien|assez bien)|avec (?:les )?honneurs?|honou?rs?|(?:summa |magna )?cum laude)\b/;
    const outputWithoutOperationalManagement = output.replace(
        /\b(?:booking|bookings|reservation|reservations|billing|invoice|invoices|invoicing)\s+management\b/g,
        ' ',
    );
    const outputHasManagerialClaim = managerialOutputPattern.test(outputWithoutOperationalManagement)
        || teamLeadershipPattern.test(output)
        || teamOrganizationPattern.test(output);
    const sourceHasManagerialClaim = hasNarratedCvAffirmedSensitiveClaim(source, managerialSourcePattern)
        || hasNarratedCvAffirmedSensitiveClaim(source, managerialResponsibilityPattern)
        || hasNarratedCvAffirmedSensitiveClaim(source, teamLeadershipPattern)
        || hasNarratedCvAffirmedSensitiveClaim(source, teamOrganizationPattern);
    const outputHasOperationalResponsibility = operationalResponsibilityOutputPattern.test(output);
    const sourceHasOperationalResponsibility = hasNarratedCvAffirmedSensitiveClaim(source, operationalResponsibilitySourcePattern)
        || hasNarratedCvAffirmedSensitiveClaim(source, managerialSourcePattern);
    return (!expertisePattern.test(output) || hasNarratedCvAffirmedSensitiveClaim(source, expertisePattern))
        && (!bilingualPattern.test(output) || hasNarratedCvAffirmedSensitiveClaim(source, bilingualPattern))
        && (!outputHasManagerialClaim || sourceHasManagerialClaim)
        && (!outputHasOperationalResponsibility || sourceHasOperationalResponsibility)
        && (!distinctionPattern.test(output) || hasNarratedCvAffirmedSensitiveClaim(source, distinctionPattern));
};

const hasNarratedCvClauseGrounding = (value = '', sourceValue = '') => {
    const clauses = getNarratedCvOutputClauses(value);
    return clauses.length > 0 && clauses.every((clause) =>
        isNarratedCvItemGrounded(clause, sourceValue, {
            minimumMatches: 2,
            minimumOutputRatio: 0.6,
        })
        && hasNarratedCvSensitiveClaimGrounding(clause, sourceValue)
    );
};

const hasNarratedCvGranularCollectionGrounding = (items = [], sourceSegments = []) => {
...
const isNarratedCvWorkIdentityLeadIn = (value = '') => {
    const source = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    return /\b(?:j ai travaille|je travaillais|j exercais|mon employeur|l entreprise|la societe|l etablissement|elle s appelait|il s appelait|nommee?|appelee?|i worked|i was working|my employer|the (?:company|business|employer|workplace) was (?:called|named)|it was (?:called|named))\b/.test(source);
};

const getNarratedCvRecordIdentityContext = (record, records = [], workRecords = []) => {
...
const hasNarratedCvFollowUpReference = (value = '') => {
    const source = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    const residenceExplanation = /\b(?:j habite|je vis|je reside|je suis domiciliee?|i live|i reside|i am based)\b[\s\S]{0,160}\b(?:car|parce que|puisque|quand|lorsque|because|when)\s+(?:j y|je|i)\s+(?:travaille|travail|work|works)\b/.test(source);
    if (residenceExplanation) return false;
    if (isNarratedCvSummaryQualitySource(value)) return false;
    return /\b(?:pour mon (?:emploi|poste|travail) actuel|pour ce(?:t)? (?:emploi|poste|travail)|dans ce(?:t)? (?:emploi|poste|travail)|j y|je fais aussi|je forme aussi|for my current (?:job|role|work)|for this (?:job|role|work)|in this (?:job|role|work)|in that (?:job|role|work)|i also)\b/.test(source);
};
...
const isNarratedCvLanguageStatement = (value = '') => {
    const source = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    const mentionsLanguage = /\b(?:langue|langues|language|languages|francais|anglais|arabe|espagnol|italien|allemand|portugais|neerlandais|chinois|japonais|polonais|french|english|arabic|spanish|italian|german|portuguese|dutch|chinese|japanese|polish)\b/.test(source);
    const givesLanguageLevel = /\b(?:je parle|pour l anglais|niveau|maternelle|bilingue|courant(?:e)?|couramment|intermediaire|debutant(?:e)?|notions?|i speak|native|bilingual|fluent|intermediate|beginner|basic)\b/.test(source);
    return mentionsLanguage && givesLanguageLevel;
};

const isNarratedCvActivityStatement = (value = '') => {
    const source = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    if (/\b(?:loisir|loisirs|activite|activites|centres? d interet|passions?|hobbies|interests?|centers? of interest)\b/.test(source)) {
        return true;
    }
    if (!/\b(?:j aime|i (?:like|enjoy))\b/.test(source)) return false;
    return !/\b(?:travailler|travail|metier|poste|aider|accompagner|servir|resoudre|gerer|organiser|collaborer|clients?|clientele|equipe|collegues?|demandes?|reclamations?|missions?|working|work|job|help|support|serve|solve|manage|organize|collaborate|customers?|clients?|team|colleagues?|requests?|complaints?|tasks?)\b/.test(source);
};

const getNarratedCvActivityContent = (value = '') => normalize(value)
    .replace(/^(?:mes?|my)?\s*(?:loisirs?|activit[eé]s?|centres? d[’']int[eé]r[eê]t|passions?|hobbies|interests?|centers? of interest)\s*(?::|sont|sont\s*:|are|are\s*:)?\s*/i, '')
    .trim();

const getNarratedCvSkillSourceParagraphs = (records = [], pattern) => {
    const paragraphIndexes = new Set(records
        .filter(({ text }) => pattern.test(stripAccents(normalizeText(text).toLowerCase()).replace(/[’']/g, ' ')))
    ...
    return [...paragraphIndexes].map((paragraphIndex) => records
        .filter((record) => record.paragraphIndex === paragraphIndex)
        .filter(({ text }) => !isNarratedCvEditorialSegment(text) && !isNarratedCvLanguageStatement(text))
        .map(({ text }) => text)
        .join(' ')
    ).filter(Boolean);
};
...
const NARRATED_CV_SKILL_STOP_TOKENS = new Set([
    'afin', 'ainsi', 'alors', 'aussi', 'aux', 'avec', 'bonne', 'bonnes', 'bon', 'bons', 'capacite',
    'capacites', 'ce', 'ces', 'cet', 'cette', 'chez', 'comme', 'competence', 'competences',
    'connaissance', 'connaissances', 'dans', 'de', 'des', 'du', 'elle', 'elles', 'en', 'est', 'et',
    'il', 'ils', 'je', 'la', 'le', 'les', 'mon', 'ma', 'mes', 'niveau', 'notre', 'nos', 'nous',
    'par', 'pour', 'pratique', 'pratiques', 'qualite', 'qualites', 'sa', 'ses', 'son', 'sur', 'une',
    'un', 'vos', 'votre', 'vous',
    'a', 'also', 'an', 'and', 'as', 'at', 'by', 'for', 'from', 'good', 'her', 'his', 'i', 'in',
    'into', 'my', 'of', 'on', 'our', 'over', 'skill', 'skills', 'the', 'their', 'to', 'via', 'well', 'with',
]);
...
const NARRATED_CV_SKILL_NUMBER_VALUES = {
    deux: '2', two: '2', trois: '3', three: '3', quatre: '4', four: '4', cinq: '5', five: '5',
    six: '6', sept: '7', seven: '7', huit: '8', eight: '8', neuf: '9', nine: '9', dix: '10', ten: '10',
    onze: '11', eleven: '11', douze: '12', twelve: '12', treize: '13', thirteen: '13',
    quatorze: '14', fourteen: '14', quinze: '15', fifteen: '15', seize: '16', sixteen: '16',
...
const normalizeNarratedCvSkillText = (value = '') => stripAccents(normalizeText(value).toLowerCase())
    .replace(/[’']/g, ' ')
    .replace(/\be[ -]?mail\b/g, ' email ')
    .replace(/\bcheck[ -]?ins?\b/g, ' arrivee ')
    .replace(/\bcheck[ -]?outs?\b/g, ' depart ')
...
const normalizeNarratedCvSkillToken = (value = '') => {
    const rawToken = stripAccents(normalizeText(value).toLowerCase()).replace(/^\.+|\.+$/g, '');
    const token = getNarratedCvTokenStem(rawToken);
    if (NARRATED_CV_SKILL_CONCEPT_TOKENS.has(rawToken)) return rawToken;
    if (/^(?:gestion|gere|gerer|gerant|gerais|gerait|gerons|gerez|gerent|occupe|traitement|traiter|traite|traites|suivi|suivre|suit|management|manage|manages|managed|managing|handle|handles|handled|handling|monitor|monitors|monitored|monitoring|tracking)$/.test(rawToken)) {
        return 'cvresponsibility';
    }
    if (/^(?:repond|repondre|repondu|reponse|reponses|reponds|repondez|respond|responds|responded|responding|response|responses|answer|answers|answered|answering)$/.test(rawToken)) {
        return 'cvrequestresponse';
    }
    if (/^(?:passage|passation|passer|relais|relayer|relaye|transmet|transmets|transmettre|transmis|transmission|handover|handovers|relay|relays|relayed|relaying)$/.test(rawToken)) {
        return 'cvhandover';
    }
    ...
    if (/^(?:telephone|telephones|phone|phones)$/.test(rawToken)) return 'telephone';
    if (/^(?:consigne|consignes|instruction|instructions)$/.test(rawToken)) return 'consigne';
    if (/^(?:equipe|equipes|team|teams)$/.test(rawToken)) return 'equipe';
    if (/^(?:nuit|nuits|night|nights)$/.test(rawToken)) return 'nuit';
    if (/^(?:logiciel|logiciels|software)$/.test(rawToken)) return 'logiciel';
    if (/^(?:hotelier|hoteliere|hoteliers|hotelieres|hotel|hotels|hospitality)$/.test(rawToken)) return 'hotelier';
    if (/^(?:reservation|reservations|reserver|booking|bookings)$/.test(rawToken)) return 'reservation';
    if (/^(?:facture|factures|facturation|billing|invoice|invoices|invoicing)$/.test(rawToken)) return 'facturation';
    if (/^(?:reclamation|reclamations|plainte|plaintes|complaint|complaints)$/.test(rawToken)) return 'reclamation';
    if (/^(?:calme|calm|calmly)$/.test(rawToken)) return 'calme';
    if (/^(?:arrivee|arrivees|arrival|arrivals|checkin)$/.test(rawToken)) return 'arrivee';
    if (/^(?:depart|departs|departure|departures|checkout)$/.test(rawToken)) return 'depart';
    if (/^(?:preparation|preparations|preparer|prepare|preparee|prepares|preparees)$/.test(rawToken)) return 'preparation';
    if (/^(?:conseil|conseils|conseiller|conseille|conseillee|conseilles|conseillees)$/.test(rawToken)) return 'conseil';
    if (/^(?:orientation|orientations|orienter|oriente|orientee|orientes|orientees)$/.test(rawToken)) return 'orientation';
    if (/^(?:coordination|coordinations|coordonner|coordonne|coordonnee|coordonnes|coordonnees)$/.test(rawToken)) return 'coordination';
    if (/^(?:encaissement|encaissements|encaisser|encaisse|encaissee|encaisses|encaissees)$/.test(rawToken)) return 'encaissement';
    return token;
};

const narratedCvSkillTokensMatch = (left = '', right = '') =>
    Boolean(left && right && normalizeNarratedCvSkillToken(left) === normalizeNarratedCvSkillToken(right));

const isNarratedCvSkillMentionAffirmed = (source = '', matchIndex = 0, matchLength = 0) => {
    const mention = normalizeNarratedCvSkillToken(source.slice(matchIndex, matchIndex + matchLength));
    const mentionPrefix = source.slice(Math.max(0, matchIndex - 48), matchIndex);
    const isWithoutLosingCalm = mention === 'calme'
        && /\bsans\s+(?:jamais\s+)?perdre(?:\s+\w+){0,3}\s*$/.test(mentionPrefix);
    if (!isWithoutLosingCalm && isNarratedCvSensitiveClaimNegated(source, matchIndex, matchLength)) return false;

    const boundaries = /[,.;&!?]|\b(?:mais|but|however|cependant|toutefois)\b/g;
    let clauseStart = 0;
    let clauseEnd = source.length;
    for (const boundary of source.matchAll(boundaries)) {
        if (boundary.index < matchIndex) clauseStart = boundary.index + boundary[0].length;
        if (boundary.index > matchIndex) {
            clauseEnd = boundary.index;
            break;
        }
    }
    const clause = source.slice(clauseStart, clauseEnd).trim();
    return !/\b(?:pas|jamais|aucun(?:e)?|sauf|hormis|excepte|ni|do not|don t|cannot|can t|not|never|except)\b/.test(clause)
        && !/\b(?:contrairement a|a l exception de|mis a part|unlike|apart from|with the exception of|loin de maitris\w*|far from master\w*)\b/.test(clause)
        && !/\b(?:maitris\w*\s+(?:mal|a peine)|barely master\w*|poorly master\w*)\b/.test(clause)
        && !/\b(?:je suis en train d apprendre|je suis en phase d apprentissage|j apprends?|i am learning|i m learning|currently learning|i am getting familiar with|i m getting familiar with)\b/.test(clause)
        && !/\b(?:je sais\s+(?:juste\s+|seulement\s+)?que|je sais a quoi sert|i know that|i know about|i am aware of|know(?:s|n)? only that)\b/.test(clause)
        && !/\b(?:j ai\s+)?(?:cesse|arrete|stoppe)\w*\b[\s\S]{0,40}\b(?:d\s+)?utilis\w*\b/.test(clause)
        && !/\b(?:j ai\s+)?(?:abandonne|renonce)\w*\b/.test(clause)
        && !/\b(?:i (?:have )?)?(?:stopped|ceased)\b[\s\S]{0,40}\b(?:use|using|work\w* with)\b/.test(clause)
        && !/\b(?:i (?:have )?)?gave up\b/.test(clause)
        && !/\b(?:souhait\w*|voudr\w*|veux|aimer\w*|envisag\w*|dois|devr\w*|prevo\w*|compte|vais|va|objectif|want|plan|need|goal)\b[\s\S]{0,64}\b(?:apprendre|acquerir|decouvrir|developper|(?:me\s+)?former|learn|acquire|discover|develop|train)\b/.test(clause)
        && !/\b(?:souhait\w*|voudr\w*|veux|aimer\w*|envisag\w*|prevo\w*|compte|vais|va|objectif|cherche|vise|pourrai|want|plan|goal|aim)\b[\s\S]{0,64}\b(?:devenir|pouvoir|cvteamwork|cvresponsibility|cvrequestresponse|cvhandover|cvorganization|cvuse|cvmastery|utiliser|maitriser|travailler|gerer|work|manage|use|master)\b/.test(clause)
        && !/\b(?:a apprendre|to learn|hors de (?:(?:mes|mon|ma|my) )?(?:champ de )?competences|non maitrise|not proficient|not skilled|totalement inconnu(?:e)?|unknown to me|m est etranger(?:e)?|m est inconnu(?:e)?|uniquement de nom|seulement de nom|only by name|oublie (?:comment |a )?|forgot (?:how |to )?|perdu (?:ma |mon |mes |la |le )?maitrise|lost (?:my |the )?mastery)\b/.test(clause)
        && !/\b(?:incapable|unable)\b[\s\S]{0,40}\b(?:utiliser|maitriser|use|master)\b/.test(clause);
};

const getNarratedCvSkillTokenRecords = (sourceValue = '', { checkAffirmation = true } = {}) => {
    const source = normalizeNarratedCvSkillText(sourceValue);
    return [...source.matchAll(/[a-z0-9+#.]+/g)]
        .map((match) => ({
            rawToken: match[0].replace(/^\.+|\.+$/g, ''),
            matchIndex: match.index,
            matchLength: match[0].length,
        }))
        .filter(({ rawToken }) => rawToken.length >= 2)
        .filter(({ rawToken }) => !/^\d+$/.test(rawToken))
        .filter(({ rawToken }) => !NARRATED_CV_MONTH_TOKENS.has(rawToken))
        .filter(({ rawToken }) => !NARRATED_CV_SKILL_STOP_TOKENS.has(rawToken))
        .map(({ rawToken, matchIndex, matchLength }) => ({
            token: normalizeNarratedCvSkillToken(rawToken),
            affirmed: !checkAffirmation || isNarratedCvSkillMentionAffirmed(source, matchIndex, matchLength),
        }))
        .filter(({ token }) => token && !NARRATED_CV_SKILL_STOP_TOKENS.has(token));
};

const hasNarratedCvPositiveSkillCue = (value = '', kind = 'generic') => {
    const source = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    if (kind === 'explicit') {
        return /\b(?:je sais|je maitrise|j utilise|mes competences|outils?|savoir faire|i can|i know|i use|i master|my skills?|tools?)\b/.test(source);
    }
    if (kind === 'quality') {
        return /\b(?:je suis|(?:mes\s+)?qualites?|i am|my qualities)\b/.test(source);
    }
    return true;
};

const getNarratedCvSkillFactSegments = (sourceSegments = []) => {
    const facts = new Map();
    (Array.isArray(sourceSegments) ? sourceSegments : [sourceSegments]).forEach((entry) => {
        const sourceValue = typeof entry === 'string' ? entry : entry && entry.text;
        const kind = typeof entry === 'string' ? 'generic' : normalize(entry && entry.kind) || 'generic';
        const fullContext = stripAccents(normalizeText(sourceValue).toLowerCase()).replace(/[’']/g, ' ');
        normalizeNarratedCvSourceText(sourceValue)
            .split(/\r?\n+|(?<=[.!?;])\s+/)
            .forEach((sentence) => {
                const normalized = stripAccents(normalizeText(sentence).toLowerCase()).replace(/[’']/g, ' ');
                const actionStart = '(?:(?:je|j|i)\\s+)?(?:m\\s+occupe|sais\\w*|maitris\\w*|utilis\\w*|know\\w*|master\\w*|use\\w*|gere\\w*|manage\\w*|handle\\w*|repond\\w*|respond\\w*|answer\\w*|transm\\w*|relay\\w*|organis\\w*|organiz\\w*|prioriti[sz]\\w*|travaill\\w*|work\\w*|scan\\w*|imprim\\w*|print\\w*|class\\w*|fil\\w*|accueil|gestion|encaissement|reservation|reception|preparation|conseil|passation|relais)';
                const infinitiveActionStart = '(?:[a-z]{3,}(?:er|ir|re)|scan|print|file)';
                const sentenceHasPositiveCue = hasNarratedCvPositiveSkillCue(normalized, kind);
                normalized
                    .replace(new RegExp(`[,;]\\s+(?=${actionStart}\\b)`, 'g'), '\n')
                    .replace(new RegExp(`\\s+(?:et|and)\\s+(?=(?:${actionStart}|${infinitiveActionStart})\\b)`, 'g'), '\n')
                    .split(/\n+/)
                    .map((text) => text.trim())
                    .filter(Boolean)
                    .forEach((text) => {
                        if (['explicit', 'quality'].includes(kind) && !sentenceHasPositiveCue) return;
                        const key = `${kind}\u0000${text}`;
                        if (!facts.has(key)) facts.set(key, { text, kind, context: fullContext });
                    });
            });
    });
    return [...facts.values()];
...
const hasNarratedCvOrderedSkillTokenGrounding = (outputTokens = [], sourceValue = '') => {
    const sourceRecords = getNarratedCvSkillTokenRecords(sourceValue);
    if (!outputTokens.length || sourceRecords.length < outputTokens.length) return false;
    return sourceRecords.some((_, startIndex) => outputTokens.every((token, outputIndex) => {
        const sourceRecord = sourceRecords[startIndex + outputIndex];
        return Boolean(sourceRecord)
            && sourceRecord.affirmed
            && narratedCvSkillTokensMatch(token, sourceRecord.token);
    }));
};
...
const getNarratedCvSkillConcepts = (value = '', { checkAffirmation = false } = {}) => {
    const records = getNarratedCvSkillTokenRecords(value, { checkAffirmation });
    const tokens = new Set(records.filter(({ affirmed }) => affirmed).map(({ token }) => token));
    const concepts = new Set([...NARRATED_CV_SKILL_CONCEPT_TOKENS].filter((token) => tokens.has(token)));
    if (['logiciel', 'reservation', 'hotelier'].every((token) => tokens.has(token))) {
...
const getNarratedCvMasteredDetailTokens = (value = '') => {
    const source = normalizeNarratedCvSkillText(value);
    const masteredTokens = new Set();
    const addDetailTokens = (text = '') => getNarratedCvSkillTokenRecords(text)
        .filter(({ affirmed, token }) => affirmed && !NARRATED_CV_SKILL_CONCEPT_TOKENS.has(token))
        .forEach(({ token }) => masteredTokens.add(token));

    for (const match of source.matchAll(/\b([a-z0-9+#.]+)\s*\(\s*(?:maitris\w*|mastery|mastered?)\b/g)) {
        addDetailTokens(match[1]);
    }

    const masteryPattern = /\b(?:maitris\w*|master(?:ed|y|s)?)\b/g;
    for (const match of source.matchAll(masteryPattern)) {
        if (/\(\s*$/.test(source.slice(Math.max(0, match.index - 3), match.index))) continue;
        const tail = source.slice(match.index + match[0].length);
        const stopIndexes = [];
        const punctuation = tail.search(/[.;!?()]/);
        if (punctuation >= 0) stopIndexes.push(punctuation);
        const nextAction = tail.search(/\b(?:et|and|mais|but|however|cependant|toutefois)\s+(?:(?:je|j|i)\s+)?(?:sais\w*|utilis\w*|use\w*|know\w*|debut\w*|begin\w*|ai\b|have\b|apprends?\b|learn\w*)/);
        if (nextAction >= 0) stopIndexes.push(nextAction);
        const tailEnd = stopIndexes.length ? Math.min(...stopIndexes) : tail.length;
        addDetailTokens(tail.slice(0, tailEnd));
    }
    return masteredTokens;
};

const hasNarratedCvMasteryDetailGrounding = (outputDetailTokens = [], sourceValue = '') => {
    const masteredTokens = getNarratedCvMasteredDetailTokens(sourceValue);
    return outputDetailTokens.length > 0 && outputDetailTokens.every((token) => masteredTokens.has(token));
};

const hasNarratedCvConceptualSkillGrounding = (item = '', sourceFacts = []) => {
    const outputRecords = getNarratedCvSkillTokenRecords(item, { checkAffirmation: false });
    ...
    const outputDetailTokens = outputRecords
        .map(({ token }) => token)
        .filter((token) => !coveredOutputTokens.has(token));

    return sourceFacts.some((sourceFactRecord) => {
        ...
        const sourceSupportsConcept = (concept) => {
            ...
            if (concept === 'cvresponsibility' && outputDetailTokens.includes('demande')) {
                return sourceConcepts.has('cvrequestresponse');
            }
            if (concept === 'cvresponsibility' && outputDetailTokens.includes('priorite')) {
                return sourceConcepts.has('cvorganization');
            }
            return false;
        };
        if (![...outputConcepts].every(sourceSupportsConcept)) return false;
        if (outputConcepts.has('cvmastery')
            && !hasNarratedCvMasteryDetailGrounding(outputDetailTokens, sourceFact)) return false;
        const sourceRecords = getNarratedCvSkillTokenRecords(sourceFact)
            .filter(({ affirmed }) => affirmed);
        const sourceDetailRecords = sourceRecords
            .filter(({ token }) => !NARRATED_CV_SKILL_CONCEPT_TOKENS.has(token));
        const contextualTokens = new Set(getNarratedCvSkillTokenRecords(sourceFactRecord.context)
            .filter(({ affirmed }) => affirmed)
            .map(({ token }) => token));
        const locallyGroundedDetailTokens = outputDetailTokens.filter((token) =>
            token !== 'hotelier'
            || sourceDetailRecords.some((record) => narratedCvSkillTokensMatch(token, record.token))
        );
        const detailsPreserveOrder = locallyGroundedDetailTokens.every((token) => {
            const startIndex = sourceDetailRecords.findIndex((record) => !record.consumed
                && narratedCvSkillTokensMatch(token, record.token));
            if (startIndex < 0) return false;
            sourceDetailRecords.slice(0, startIndex + 1).forEach((record) => { record.consumed = true; });
            return true;
        });
        if (!detailsPreserveOrder) return false;
        const domainQualifiersGrounded = outputDetailTokens
            .filter((token) => token === 'hotelier')
            .every((token) => contextualTokens.has(token));
        return domainQualifiersGrounded
            && getNarratedCvYears(item).every((year) => new Set(getNarratedCvYears(sourceFact)).has(year))
            && hasNarratedCvSensitiveClaimGrounding(item, sourceFact)
            && hasNarratedCvSkillNestedPairGrounding(item, sourceFact)
            && hasNarratedCvSkillConnectorPairGrounding(item, sourceFact)
            && hasNarratedCvSkillQuantityGrounding(item, sourceFact);
    });
};

const getNarratedCvSkillQuantityPairs = (value = '') => {
    const source = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    const numberPattern = '(?:\\d+(?:[.,]\\d+)?|deux|trois|quatre|cinq|six|sept|huit|neuf|dix|onze|douze|treize|quatorze|quinze|seize|vingt|trente|quarante|cinquante|soixante|cent|mille|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|twenty|thirty|forty|fifty|sixty|hundred|thousand)';
    const matcher = new RegExp(`\\b(${numberPattern})\\s+([a-z][a-z-]{1,})\\b`, 'g');
    return [...source.matchAll(matcher)].map((match) => ({
        number: NARRATED_CV_SKILL_NUMBER_VALUES[match[1]] || match[1].replace(',', '.'),
        noun: normalizeNarratedCvSkillToken(match[2]),
    }));
};

const hasNarratedCvSkillQuantityGrounding = (value = '', sourceValue = '') => {
    const outputPairs = getNarratedCvSkillQuantityPairs(value);
    const outputNumbers = stripAccents(normalizeText(value).toLowerCase())
        .match(/\b(?:\d+(?:[.,]\d+)?|deux|trois|quatre|cinq|six|sept|huit|neuf|dix|onze|douze|treize|quatorze|quinze|seize|vingt|trente|quarante|cinquante|soixante|cent|mille|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|twenty|thirty|forty|fifty|sixty|hundred|thousand)\b/g) || [];
    if (outputNumbers.length !== outputPairs.length) return outputNumbers.length === 0;
    const sourcePairs = getNarratedCvSkillQuantityPairs(sourceValue);
    return outputPairs.every((outputPair) => sourcePairs.some((sourcePair) =>
        outputPair.number === sourcePair.number
        && narratedCvSkillTokensMatch(outputPair.noun, sourcePair.noun)
    ));
};

const NARRATED_CV_SKILL_RELATIONAL_TOKENS = new Set([
    'arrivee', 'badge', 'client', 'commande', 'courrier', 'demande', 'depart', 'facturation', 'livre',
    'fournisseur', 'passeport', 'reclamation', 'reservation', 'salle', 'standard',
]);

const getNarratedCvSkillNestedPairs = (value = '') => {
    const source = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    return [...source.matchAll(/(?=\b([a-z][a-z-]{2,})\s+(?:de|d|des|du|of)\s+(?:\d+(?:[.,]\d+)?\s+)?([a-z][a-z-]{2,})\b)/g)]
        .map((match) => ({
            parent: normalizeNarratedCvSkillToken(match[1]),
            child: normalizeNarratedCvSkillToken(match[2]),
        }))
        .filter(({ parent, child }) =>
            NARRATED_CV_SKILL_RELATIONAL_TOKENS.has(parent)
            && NARRATED_CV_SKILL_RELATIONAL_TOKENS.has(child)
        );
};

const hasNarratedCvSkillNestedPairGrounding = (value = '', sourceValue = '') => {
    const outputPairs = getNarratedCvSkillNestedPairs(value);
    if (!outputPairs.length) return true;
    const sourcePairs = getNarratedCvSkillNestedPairs(sourceValue);
    return outputPairs.every((outputPair) => sourcePairs.some((sourcePair) =>
        outputPair.parent === sourcePair.parent && outputPair.child === sourcePair.child
    ));
};

const getNarratedCvSkillConnectorPairs = (value = '') => {
    const source = normalizeNarratedCvSkillText(value);
    const connectorPairs = [...source.matchAll(/\b([a-z0-9+#.]+)\s+(?:comme|as|pour|for|en|in)\s+(?:(?:un|une|le|la|les|des|du|de|l|a|an|the)\s+)?([a-z0-9+#.]+)\b/g)];
    const appositionPairs = [...source.matchAll(/\b([a-z0-9+#.]+)\s*,\s*(?:(?:un|une|le|la|l|a|an|the)\s+)?(logiciel|software)\b/g)];
    return [...connectorPairs, ...appositionPairs]
        .map((match) => ({
            parent: normalizeNarratedCvSkillToken(match[1]),
            child: normalizeNarratedCvSkillToken(match[2]),
        }))
        .filter(({ parent, child }) => parent && child
            && !NARRATED_CV_SKILL_CONCEPT_TOKENS.has(parent)
            && !NARRATED_CV_SKILL_CONCEPT_TOKENS.has(child)
            && !NARRATED_CV_SKILL_STOP_TOKENS.has(parent)
            && !NARRATED_CV_SKILL_STOP_TOKENS.has(child));
};

const hasNarratedCvSkillConnectorPairGrounding = (value = '', sourceValue = '') => {
    const outputPairs = getNarratedCvSkillConnectorPairs(value);
    if (!outputPairs.length) return true;
    const sourcePairs = getNarratedCvSkillConnectorPairs(sourceValue);
    return outputPairs.every((outputPair) => sourcePairs.some((sourcePair) =>
        outputPair.parent === sourcePair.parent && outputPair.child === sourcePair.child
    ));
};

const hasNarratedCvExplicitCompetenceDenial = (item = '', sourceValue = '') => {
    const source = stripAccents(normalizeText(sourceValue).toLowerCase()).replace(/[’']/g, ' ');
    const outputTokens = new Set(getNarratedCvSkillTokenRecords(item, { checkAffirmation: false })
        .map(({ token }) => token));
    const outputDetails = new Set([...outputTokens]
        .filter((token) => !NARRATED_CV_SKILL_CONCEPT_TOKENS.has(token)));
    const relevantOutputTokens = outputDetails.size ? outputDetails : outputTokens;
    const denialPattern = /\b(?:n est pas|ne fait pas partie|n appartient pas|is not|isn t|not one of)\b[\s\S]{0,48}\b(?:mes |my )?(?:competences?|skills?)\b/g;
    const boundaries = /[,.;!?]|\b(?:mais|but|however|cependant|toutefois)\b/g;
    const hasRelevantToken = (value = '') => getNarratedCvSkillTokenRecords(value, { checkAffirmation: false })
        .some(({ token }) => relevantOutputTokens.has(token));

    return [...source.matchAll(denialPattern)].some((denial) => {
        const priorBoundaries = [...source.slice(0, denial.index).matchAll(boundaries)];
        const clauseStart = priorBoundaries.length
            ? priorBoundaries[priorBoundaries.length - 1].index + priorBoundaries[priorBoundaries.length - 1][0].length
            : 0;
        const nextBoundary = [...source.slice(denial.index + denial[0].length).matchAll(boundaries)][0];
        const clauseEnd = nextBoundary
            ? denial.index + denial[0].length + nextBoundary.index
            : source.length;
        const clause = source.slice(clauseStart, clauseEnd);
        if (hasRelevantToken(clause)) return true;

        const denialPrefix = source.slice(clauseStart, denial.index).trim();
        if (!/^(?:ce|cela|ca|this|it)$/.test(denialPrefix)) return false;
        const previousBoundaryEnd = clauseStart > 0 ? clauseStart - 1 : 0;
        const earlierBoundaries = [...source.slice(0, previousBoundaryEnd).matchAll(boundaries)];
        const previousClauseStart = earlierBoundaries.length
            ? earlierBoundaries[earlierBoundaries.length - 1].index + earlierBoundaries[earlierBoundaries.length - 1][0].length
            : 0;
        return hasRelevantToken(source.slice(previousClauseStart, previousBoundaryEnd));
    });
};

const hasNarratedCvAnaphoricSkillRetraction = (item = '', sourceValue = '') => {
    const source = stripAccents(normalizeText(sourceValue).toLowerCase()).replace(/[’']/g, ' ');
    const outputTokens = new Set(getNarratedCvSkillTokenRecords(item, { checkAffirmation: false })
        .map(({ token }) => token));
    const outputDetails = new Set([...outputTokens]
        .filter((token) => !NARRATED_CV_SKILL_CONCEPT_TOKENS.has(token)));
    const relevantOutputTokens = outputDetails.size ? outputDetails : outputTokens;
    const retractionPattern = /\b(?:mais\s+)?(?:(?:ce|cela|ca)\s+n est\s+(?:desormais\s+)?plus\s+(?:le cas|vrai)|but\s+(?:not\s+anymore|no\s+longer)|(?:j ai\s+)?(?:cesse|arrete)\w*\s+de\s+m\s+en\s+servir)\b/g;
    return [...source.matchAll(retractionPattern)].some((retraction) => {
        const previousSentenceStart = Math.max(
            source.lastIndexOf('.', retraction.index - 1),
            source.lastIndexOf(';', retraction.index - 1),
            source.lastIndexOf('!', retraction.index - 1),
            source.lastIndexOf('?', retraction.index - 1),
        ) + 1;
        const previousClause = source.slice(previousSentenceStart, retraction.index).replace(/[,\s]+$/, '');
        return getNarratedCvSkillTokenRecords(previousClause, { checkAffirmation: false })
            .some(({ token }) => relevantOutputTokens.has(token));
    });
};

const hasNarratedCvSkillContradiction = (item = '', allSourceFacts = []) => {
    const outputTokens = new Set(getNarratedCvSkillTokenRecords(item, { checkAffirmation: false })
        .map(({ token }) => token));
    if (!outputTokens.size) return false;
    const outputConcepts = getNarratedCvSkillConcepts(item);
    const outputDetails = new Set([...outputTokens]
        .filter((token) => !NARRATED_CV_SKILL_CONCEPT_TOKENS.has(token)));
    const contradictedTokens = outputDetails.size ? outputDetails : outputTokens;
    const outputNestedPairs = getNarratedCvSkillNestedPairs(item);
    const nestedOutputTokens = new Set(outputNestedPairs
        .flatMap(({ parent, child }) => [parent, child]));
    const clauseContradicts = (sourceValue = '') => normalizeNarratedCvSkillText(sourceValue)
        .split(/[,.;!?]+|\b(?:mais|but|however|cependant|toutefois)\b/)
        .map((clause) => clause.trim())
        .filter(Boolean)
        .some((clause) => {
            const negatedTokens = getNarratedCvSkillTokenRecords(clause)
                .filter(({ affirmed, token }) => !affirmed && contradictedTokens.has(token))
                .map(({ token }) => token);
            if (!negatedTokens.length) return false;
            if (!outputNestedPairs.length) return true;
            const negativePairs = getNarratedCvSkillNestedPairs(clause);
            const nestedPairContradicted = outputNestedPairs.some((outputPair) => negativePairs.some((negativePair) =>
                outputPair.parent === negativePair.parent && outputPair.child === negativePair.child
            ));
            return nestedPairContradicted
...
const hasNarratedCvSkillCollectionGrounding = (items = [], sourceSegments = [], allSourceSegments = []) => {
    ...
    const allSegments = getNarratedCvSkillFactSegments(
        (Array.isArray(allSourceSegments) ? allSourceSegments : [allSourceSegments])
    ...
    return values.every((item) => {
        ...
        return segments.some((sourceFactRecord) => {
            ...
            return hasNarratedCvOrderedSkillTokenGrounding(outputTokens, sourceValue)
                ...
                && hasNarratedCvSensitiveClaimGrounding(item, sourceValue)
                && hasNarratedCvSkillNestedPairGrounding(item, sourceValue)
...
const hasNarratedCvSkillAggregateCoverage = (items = [], sourceSegments = [], minimumRatio = 0.55) => {
    const sourceTokens = [...new Set(getNarratedCvSkillFactSegments(
        (Array.isArray(sourceSegments) ? sourceSegments : [sourceSegments])
            .map((text) => ({ text, kind: 'explicit' })),
    ).flatMap(({ text }) => getNarratedCvSkillTokenRecords(text)
        .filter(({ affirmed }) => affirmed)
        .map(({ token }) => token)))]
        .filter((token) => !['cvuse', 'cvmastery'].includes(token));
    if (!sourceTokens.length) return true;
    const outputTokens = new Set((Array.isArray(items) ? items : [])
        .flatMap((item) => getNarratedCvSkillTokenRecords(item, { checkAffirmation: false })
            .map(({ token }) => token)));
    const matches = sourceTokens.filter((token) => outputTokens.has(token)).length;
...
const cleanNarratedCvScalarClaimValue = (value = '', maxLength = 120) => {
    const candidate = normalizeText(value)
        .replace(/,\s*(?=(?:(?:et|and|mais|but|cependant|however|toutefois)\s+)?(?:je|j[’']|mon|ma|mes|i|my)\b)[\s\S]*$/iu, '')
        .replace(/\s+(?:et|and|mais|but|cependant|however|toutefois)\s+(?=(?:je|j[’']|mon|ma|mes|i|my)\b)[\s\S]*$/iu, '')
        .replace(/^[\s«»"“”']+|[\s,«»"“”']+$/g, '')
        .trim();
    const normalizedCandidate = stripAccents(candidate.toLowerCase()).replace(/[’']/g, ' ');
    return candidate.length <= maxLength
        && !/^(?:pas|not|jamais|never|aucun|aucune)\b/.test(normalizedCandidate)
        && /^[\p{L}\p{N}][\p{L}\p{N}'’ (),./+-]*$/u.test(candidate)
        ? candidate
        : '';
};

const isNarratedCvConditionalScalarClaim = (segment = '', matchIndex = 0) => {
    const prefix = stripAccents(normalizeText(segment.slice(0, matchIndex)).toLowerCase()).replace(/[’']/g, ' ');
    const localPrefix = prefix.split(/[.;!?]|\b(?:mais|but|however)\b/).pop()?.trim() || '';
    return /\b(?:si|if|peut etre|maybe|perhaps)\b[\s\S]{0,48}$/.test(localPrefix);
};

const cleanNarratedCvLocationClaimValue = (value = '') => {
    const boundedValue = normalizeText(value)
        .replace(/(?:,\s*|\s+)(?:où(?=\s|$)|where\b)[\s\S]*$/iu, '')
        .replace(/(?:,\s*|\s+)(?:et|and|mais|but|tout\s+en|while)\s+(?=(?:(?:je|j[’']|i)\s+)?(?:travail(?:le|les|lons|lez|lent)|travaillant|exerce|[eé]tudie|work(?:s|ing)?|study|studies|suis\s+(?:salari[eé]e?|en\s+t[eé]l[eé]travail)|am\s+(?:employed|working\s+remotely))\b)[\s\S]*$/iu, '')
        .replace(/\s+(?:car|parce\s+que|puisque|because|as)\s+(?=(?:(?:j[’']?y|je|j[’']|i)\s+)?(?:travail(?:le|les|lons|lez|lent)|travaillant|exerce|[eé]tudie|work(?:s|ing)?|study|studies)\b)[\s\S]*$/iu, '')
        .replace(/\s+(?:car|parce\s+que|puisque|because|as)\s+(?=(?:mon|ma|my)\s+(?:travail|emploi|work|job)\s+(?:est|se\s+trouve|is)\b)[\s\S]*$/iu, '')
        .replace(/\s+(?:quand|lorsque|when)\s+(?=(?:(?:j[’']?y|je|j[’']|i)\s+)?(?:travail(?:le|les|lons|lez|lent)|travaillant|exerce|[eé]tudie|work(?:s|ing)?|study|studies)\b)[\s\S]*$/iu, '')
        .replace(/\s+(?:pour\s+[eê]tre|to\s+be)\s+(?:(?:plus\s+)?(?:proche|pr[eè]s)\s+de\s+(?:ma|mon|mes|notre|nos)\s+famille|(?:closer|close|near)\s+to\s+(?:my|our)\s+family)\b[\s\S]*$/iu, '')
        .replace(/\s+(?:en\s+t[eé]l[eé]travail|while\s+employed|(?:proche|pr[eè]s)\s+de\s+(?:ma|mon|mes|notre|nos)\s+famille|near\s+(?:my|our)\s+family)\b[\s\S]*$/iu, '')
        .replace(/\s+(?:pour\s+(?:(?:des?|de\s+la)\s+)?(?:raisons?|motifs?)\b|for\s+(?:(?:personal|professional|family)\s+)?reasons?\b)[\s\S]*$/iu, '')
        .replace(/\s+(?:depuis|since)\s+[\s\S]*$/iu, '')
        .replace(/\s+(?:avec|with)\s+(?=(?:(?:un|une|deux|trois|two|three)\s+)?(?:enfants?|children|famille|family)\b)[\s\S]*$/iu, '')
        .replace(/\s+(?:avec|with|pour|for)\s+(?=(?:mon|ma|mes|notre|nos|my|our|the|le|la|les|famille|family|travail|work)\b)[\s\S]*$/iu, '')
        .trim();
    return cleanNarratedCvScalarClaimValue(boundedValue, 120);
};

const getNarratedCvLocationClaimsFromSegment = (segment = '') => {
    const claims = [];
    const actionPattern = /\b((?:j[’']habite|je\s+(?:(?:n[’']|ne)\s*)?(?:habite|vis|r[eé]side)|je\s+(?:(?:n[’']|ne)\s*)?suis\s+(?:(?:pas|plus|jamais)\s+)?(?:domicili[eé]e?|bas[eé]e?)|i\s+(?:(?:do\s+not|don[’']t|no\s+longer|never)\s+)?(?:live|reside)|i\s+am\s+(?:(?:not|no\s+longer|never)\s+)?based)\s+(?:(?:pas|plus|jamais|not|anymore)\s+)?(?:[àa]|au|aux|en|sur|in|at)\s+)([^.!?;\n]{1,140}?)(?=$|[.!?;\n]|,\s*(?=(?:(?:et|and|mais|but|cependant|however|toutefois)\s+)?(?:je|j[’']|mon|ma|mes|i|my)\b))/giu;
    for (const match of String(segment || '').matchAll(actionPattern)) {
        const value = cleanNarratedCvLocationClaimValue(match[2]);
        if (!value) continue;
        const cue = stripAccents(normalizeText(match[1]).toLowerCase()).replace(/[’']/g, ' ');
        const negated = /\b(?:ne|n|pas|plus|jamais|not|never|do not|don t|no longer)\b/.test(cue);
        const conditional = isNarratedCvConditionalScalarClaim(segment, match.index);
        claims.push({
            index: match.index,
            kind: conditional ? 'uncertain' : negated ? 'negative' : 'positive',
            value,
            evidence: normalizeText(segment),
        });
    }

    const labelPattern = /\b(?:adresse|domicile|ville|localisation|lieu\s+de\s+r[eé]sidence|home\s+address|location)\s*:\s*([^.!?;\n]{1,140}?)(?=$|[.!?;\n]|,\s*(?=(?:(?:et|and|mais|but|cependant|however|toutefois)\s+)?(?:je|j[’']|mon|ma|mes|i|my)\b))/giu;
    for (const match of String(segment || '').matchAll(labelPattern)) {
        const value = cleanNarratedCvScalarClaimValue(match[1], 120);
        if (!value) continue;
        claims.push({
            index: match.index,
            kind: isNarratedCvConditionalScalarClaim(segment, match.index) ? 'uncertain' : 'positive',
            value,
            evidence: normalizeText(segment),
        });
    }

    const uncertainPattern = /\b(?:je\s+(?:souhaite|voudrais|aimerais|pr[eé]vois|compte|vais)\s+(?:[eê]tre\s+)?(?:domicili[eé]e?|habiter|vivre|r[eé]sider)|i\s+(?:want|would\s+like|plan|intend)\s+to\s+(?:be\s+based|live|reside))\s+(?:[àa]|au|aux|en|in|at)\s+([^.!?;\n]{1,140}?)(?=$|[.!?;\n]|,\s*(?=(?:(?:et|and|mais|but|cependant|however|toutefois)\s+)?(?:je|j[’']|mon|ma|mes|i|my)\b))/giu;
    for (const match of String(segment || '').matchAll(uncertainPattern)) {
        const value = cleanNarratedCvScalarClaimValue(match[1], 120);
        if (!value) continue;
        claims.push({
            index: match.index,
...
const NARRATED_CV_PERMIT_NAME_PATTERN = '(?:permis(?:\\s+de\\s+conduire)?|driving\\s+licen[cs]e(?:s)?|driver[’\']?s\\s+licen[cs]e(?:s)?)';
const NARRATED_CV_PERMIT_CLASS_PATTERN = '(?:AM|A1|A2|A|B1|BE|B|C1E|C1|CE|C|D1E|D1|DE|D)(?:\\s*(?:[,/+]|et|and|ainsi\\s+que)\\s*(?:(?:le|la|the|mon|ma|my|our)\\s+)?(?:(?:permis(?:\\s+de\\s+conduire)?|driving\\s+licen[cs]e)\\s+)?(?:AM|A1|A2|A|B1|BE|B|C1E|C1|CE|C|D1E|D1|DE|D))*';

const getNarratedCvPermitClaimsFromSegment = (segment = '') => {
    const source = String(segment || '');
    const claims = [];
    const ownedPattern = new RegExp(
        `\\b((?:j[’']ai|je\\s+(?:(?:n[’']|ne)\\s*)?(?:ai|poss[eè]de|d[eé]tiens|dispose)|je\\s+(?:(?:n[’']|ne)\\s*)?suis\\s+(?:(?:pas|plus|jamais)\\s+)?titulaire|i\\s+(?:(?:do\\s+not|don[’']t|no\\s+longer|never|also)\\s+)?(?:have|hold))\\s+(?:(?:d[eé]sormais|dor[eé]navant)\\s+)?(?:(?:pas|plus|jamais|not|anymore)\\s+)?(?:(?:aussi|[eé]galement|also)\\s+)?(?:(?:le|la|les|un|une|du|des|de\\s+la|mon|ma|mes|notre|nos|my|our|a|an|the)\\s+)?)(${NARRATED_CV_PERMIT_NAME_PATTERN}\\s*(?:[:\\-]\\s*)?${NARRATED_CV_PERMIT_CLASS_PATTERN})\\b`,
        'giu',
    );
    for (const match of source.matchAll(ownedPattern)) {
        const value = cleanNarratedCvScalarClaimValue(match[2], 80);
        if (!value) continue;
        const cue = stripAccents(normalizeText(match[1]).toLowerCase()).replace(/[’']/g, ' ');
        const negated = /\b(?:ne|n|pas|plus|jamais|not|never|do not|don t|no longer)\b/.test(cue);
        const conditional = isNarratedCvConditionalScalarClaim(segment, match.index);
    ...
    const reverseEnglishOwnedPattern = new RegExp(
        `\\b(i\\s+(?:(?:do\\s+not|don[’']t|no\\s+longer|never)\\s+)?(?:have|hold))\\s+(?:(?:not|anymore)\\s+)?(?:a|an|the|my|our)?\\s*(${NARRATED_CV_PERMIT_CLASS_PATTERN})\\s+(driving\\s+licen[cs]e(?:s)?)\\b`,
        'giu',
    );
    for (const match of source.matchAll(reverseEnglishOwnedPattern)) {
        const cue = stripAccents(normalizeText(match[1]).toLowerCase()).replace(/[’']/g, ' ');
        const conditional = isNarratedCvConditionalScalarClaim(segment, match.index);
        claims.push({
            index: match.index,
            kind: conditional
                ? 'uncertain'
                : /\b(?:not|never|do not|don t|no longer)\b/.test(cue) ? 'negative' : 'positive',
            value: `Driving licence ${normalizeText(match[2])}`,
            evidence: normalizeText(segment),
        });
    }
    ...
    const acquiredPatterns = [
        new RegExp(
            `\\b(?:j[’']ai\\s+(?:obtenu|d[eé]croch[eé]|valid[eé]|pass[eé])|i\\s+(?:obtained|earned|passed))\\s+(?:(?:le|la|les|un|une|mon|ma|mes|my|our|a|an|the)\\s+)?(${NARRATED_CV_PERMIT_NAME_PATTERN}\\s*(?:[:\\-]\\s*)?${NARRATED_CV_PERMIT_CLASS_PATTERN})\\s*[.!]?\\s*$`,
            'giu',
        ),
        new RegExp(
            `\\b(?:j[’']ai\\s+(?:obtenu|d[eé]croch[eé]|valid[eé]|pass[eé])|i\\s+(?:obtained|earned|passed))\\b[^.!?;]{1,180}?\\b(?:et|ainsi\\s+que|and)\\s+(?:(?:le|la|les|un|une|mon|ma|mes|my|our|a|an|the)\\s+)?(${NARRATED_CV_PERMIT_NAME_PATTERN}\\s*(?:[:\\-]\\s*)?${NARRATED_CV_PERMIT_CLASS_PATTERN})\\s*[.!]?\\s*$`,
            'giu',
        ),
    ];
    for (const acquiredPattern of acquiredPatterns) {
        for (const match of source.matchAll(acquiredPattern)) {
            const value = cleanNarratedCvScalarClaimValue(match[1], 80);
            if (!value) continue;
            const valueIndex = (match.index || 0) + match[0].lastIndexOf(match[1]);
            const negated = isNarratedCvSensitiveClaimNegated(source, valueIndex, match[1].length);
            const conditional = isNarratedCvConditionalScalarClaim(segment, match.index);
            claims.push({
                index: valueIndex,
                kind: conditional ? 'uncertain' : negated ? 'negative' : 'positive',
    ...
    const continuedPermitPattern = new RegExp(
        `(?:\\b(?:et|and)\\b|,)\\s+(?:(?:le|la|les|the)\\s+)?(${NARRATED_CV_PERMIT_NAME_PATTERN}\\s*(?:[:\\-]\\s*)?${NARRATED_CV_PERMIT_CLASS_PATTERN})\\b`,
        'giu',
    );
    const continuedPermitMatches = [...source.matchAll(continuedPermitPattern)];

    const standalonePattern = new RegExp(`^\\s*(${NARRATED_CV_PERMIT_NAME_PATTERN}\\s*(?:[:\\-]\\s*)?${NARRATED_CV_PERMIT_CLASS_PATTERN})\\s*[.!]?\\s*$`, 'iu');
    const standaloneMatch = source.match(standalonePattern);
    if (standaloneMatch) {
        claims.push({
            index: standaloneMatch.index || 0,
            kind: isNarratedCvConditionalScalarClaim(segment, standaloneMatch.index || 0) ? 'uncertain' : 'positive',
            value: cleanNarratedCvScalarClaimValue(standaloneMatch[1], 80),
            evidence: normalizeText(segment),
        });
    }

    const withoutPattern = new RegExp(`\\b(?:sans|without)\\s+(?:(?:le|la|un|une|a|an|the)\\s+)?(${NARRATED_CV_PERMIT_NAME_PATTERN}(?:\\s*(?:[:\\-]\\s*)?${NARRATED_CV_PERMIT_CLASS_PATTERN})?)\\b`, 'giu');
    for (const match of source.matchAll(withoutPattern)) {
        claims.push({
            index: match.index,
            kind: 'negative',
            value: cleanNarratedCvScalarClaimValue(match[1], 80),
            generic: !new RegExp(`\\b${NARRATED_CV_PERMIT_CLASS_PATTERN}\\b`, 'iu').test(match[1]),
            evidence: normalizeText(segment),
        });
    }

    const genericNegativePattern = new RegExp(
        `\\b(?:je\\s+(?:n[’']ai|ne\\s+(?:poss[eè]de|d[eé]tiens))\\s+(?:(?:d[eé]sormais|dor[eé]navant)\\s+)?(?:pas|plus|jamais)\\s+(?:(?:de|le|du)\\s+)?|i\\s+(?:do\\s+not|don[’']t|no\\s+longer|never)\\s+(?:have|hold)\\s+(?:(?:a|the)\\s+)?)(${NARRATED_CV_PERMIT_NAME_PATTERN}(?:\\s*(?:[:\\-]\\s*)?${NARRATED_CV_PERMIT_CLASS_PATTERN})?)\\b`,
        'giu',
    );
    for (const match of source.matchAll(genericNegativePattern)) {
        const value = cleanNarratedCvScalarClaimValue(match[1], 80);
        const classKey = getNarratedCvPermitClassKey(value);
        claims.push({
            index: match.index,
            kind: 'negative',
            value,
    ...
    const expiredPattern = new RegExp(
        `\\b(${NARRATED_CV_PERMIT_NAME_PATTERN}\\s*(?:[:\\-]\\s*)?${NARRATED_CV_PERMIT_CLASS_PATTERN})\\b[^.!?;]{0,40}\\b(?:a\\s+expir[eé]|est\\s+expir[eé]|n[’']est\\s+(?:d[eé]sormais\\s+)?plus\\s+valide|n[’']est\\s+(?:d[eé]sormais\\s+)?plus\\s+en\\s+cours\\s+de\\s+validit[eé]|has\\s+expired|is\\s+expired|is\\s+no\\s+longer\\s+valid)(?=\\s|[.!?,;]|$)`,
        'giu',
    );
    for (const match of source.matchAll(expiredPattern)) {
        claims.push({
            index: match.index,
            kind: 'negative',
            value: cleanNarratedCvScalarClaimValue(match[1], 80),
    ...
    const anaphoricExpiredClassPattern = new RegExp(
        `^\\s*(?:(?:en\\s+fait|finalement|d[eé]sormais|actually|now)\\s*,?\\s*)?(?:(?:le|la|the|mon|ma|my|our)\\s+)?(${NARRATED_CV_PERMIT_CLASS_PATTERN})\\s+(?:a\\s+expir[eé]|est\\s+expir[eé]|n[’']est\\s+(?:d[eé]sormais\\s+)?plus\\s+valide|has\\s+expired|is\\s+expired|is\\s+no\\s+longer\\s+valid)(?=\\s|[.!?,;]|$)`,
        'giu',
    );
    for (const match of source.matchAll(anaphoricExpiredClassPattern)) {
        claims.push({
            index: match.index,
            kind: 'negative',
            value: `Permis ${normalizeText(match[1])}`,
            anaphoric: true,
            evidence: normalizeText(segment),
        });
    }

    const excludedClassPattern = new RegExp(
        `\\b(?:sauf|hormis|except[eé]?|excluding|except)\\s+(?:(?:le|la|les|un|une|a|an|the)\\s+)?(?:${NARRATED_CV_PERMIT_NAME_PATTERN}\\s*)?(${NARRATED_CV_PERMIT_CLASS_PATTERN})\\b`,
        'giu',
    );
    for (const match of source.matchAll(excludedClassPattern)) {
        const precedingClaim = claims
            .filter((claim) => claim.index < match.index && match.index - claim.index <= 160)
            .sort((left, right) => right.index - left.index)[0];
        if (!precedingClaim) continue;
        const french = /\b(?:sauf|hormis|excepte|permis)\b/i.test(match[0]);
        claims.push({
            index: match.index,
            kind: 'negative',
            value: `${french ? 'Permis' : 'Driving licence'} ${normalizeText(match[1])}`,
            evidence: normalizeText(segment),
        });
    }

    const explicitExclusiveClassPattern = new RegExp(
        `^\\s*(?:(?:en\\s+fait|finalement|actually|ultimately)\\s*,?\\s*)?(?:(?:j[’']ai|je\\s+(?:poss[eè]de|d[eé]tiens)|i\\s+(?:have|hold))\\s+)?(?:seulement|uniquement|only|just)\\s+(?:(?:le|la|les|un|une|a|an|the)\\s+)?(${NARRATED_CV_PERMIT_NAME_PATTERN})\\s*(?:[:\\-]\\s*)?(${NARRATED_CV_PERMIT_CLASS_PATTERN})\\s*[.!]?\\s*$`,
        'iu',
    );
    const explicitExclusiveClassMatch = source.match(explicitExclusiveClassPattern);
    if (explicitExclusiveClassMatch) {
        const french = /\b(?:seulement|uniquement|permis)\b/i.test(source);
        claims.push({
            index: explicitExclusiveClassMatch.index || 0,
            kind: 'exclusive',
            value: `${french ? 'Permis' : 'Driving licence'} ${normalizeText(explicitExclusiveClassMatch[2])}`,
            evidence: normalizeText(segment),
        });
    }

    const anaphoricExclusiveClassPattern = new RegExp(
        `^\\s*(?:(?:en\\s+fait|finalement|actually|ultimately)\\s*,?\\s*)(?:seulement|uniquement|only|just)\\s+(?:(?:le|la|les|un|une|a|an|the)\\s+)?(${NARRATED_CV_PERMIT_CLASS_PATTERN})\\s*[.!]?\\s*$`,
        'iu',
    );
    const anaphoricExclusiveClassMatch = source.match(anaphoricExclusiveClassPattern);
    if (anaphoricExclusiveClassMatch) {
        const french = /\b(?:en fait|finalement|seulement|uniquement)\b/i.test(source);
        claims.push({
            index: anaphoricExclusiveClassMatch.index || 0,
            kind: 'exclusive',
            value: `${french ? 'Permis' : 'Driving licence'} ${normalizeText(anaphoricExclusiveClassMatch[1])}`,
            anaphoric: true,
            evidence: normalizeText(segment),
        });
    }

    if (/^\s*(?:(?:en\s+fait|finalement|actually)\s*,?\s*)?(?:je\s+ne\s+l[’']ai\s+plus|i\s+no\s+longer\s+have\s+it)\s*[.!]?\s*$/iu.test(source)) {
        claims.push({
            index: 0,
            kind: 'negative',
            value: 'permis',
    ...
    const uncertainPatterns = [
        new RegExp(`\\b(?:je\\s+(?:(?:suis\\s+en\\s+train\\s+de\\s+)?(?:pr[eé]pare|passe)|(?:souhaite|pr[eé]vois|compte|vais)\\s+(?:obtenir|passer))|i\\s+(?:am\\s+)?(?:preparing|studying|applying|planning)[^.!?;]{0,24})[^.!?;]{0,40}?(${NARRATED_CV_PERMIT_NAME_PATTERN}\\s*(?:[:\\-]\\s*)?${NARRATED_CV_PERMIT_CLASS_PATTERN})\\b`, 'giu'),
        new RegExp(`\\b(${NARRATED_CV_PERMIT_NAME_PATTERN}\\s*(?:[:\\-]\\s*)?${NARRATED_CV_PERMIT_CLASS_PATTERN})\\b[^.!?;]{0,24}\\b(?:en\\s+cours|in\\s+progress)\\b`, 'giu'),
        new RegExp(`\\b(${NARRATED_CV_PERMIT_NAME_PATTERN}\\s*(?:[:\\-]\\s*)?${NARRATED_CV_PERMIT_CLASS_PATTERN})\\b[^.!?;]{0,40}\\b(?:requis|recommand[eé]|n[eé]cessaire|required|recommended|needed)\\b`, 'giu'),
    ];
    for (const uncertainPattern of uncertainPatterns) {
        for (const match of source.matchAll(uncertainPattern)) {
            claims.push({
                index: match.index,
                kind: 'uncertain',
                value: cleanNarratedCvScalarClaimValue(match[1], 80),
    ...
    for (const match of continuedPermitMatches) {
        const value = cleanNarratedCvScalarClaimValue(match[1], 80);
        const precedingClaim = claims
            .filter((claim) => claim.index < match.index)
            .sort((left, right) => right.index - left.index)[0];
        if (!value || !precedingClaim || match.index - precedingClaim.index > 160) continue;
        claims.push({
            index: match.index,
            kind: precedingClaim.kind,
            value,
            evidence: normalizeText(segment),
        });
    }
    return claims.filter(({ value }) => Boolean(value)).sort((left, right) => left.index - right.index);
};

const normalizeNarratedCvScalarClaim = (value = '') => stripAccents(normalizeText(value).toLowerCase())
    .replace(/[’']/g, ' ')
    .replace(/[^a-z0-9]+/g, ' ')
    .trim();

const getNarratedCvPermitClassKeys = (value = '') => [...String(value || '').toUpperCase()
    .matchAll(/\b(?:AM|A1|A2|A|B1|BE|B|C1E|C1|CE|C|D1E|D1|DE|D)\b/g)]
    .map((match) => match[0]);

const getNarratedCvPermitClassKey = (value = '') => getNarratedCvPermitClassKeys(value).join('|');

const resolveNarratedCvScalarClaimState = (
    documentText = '',
    getClaimsFromSegment,
    areEquivalent = (left, right) => normalizeNarratedCvScalarClaim(left) === normalizeNarratedCvScalarClaim(right),
) => {
    const claims = getNarratedCvSegmentRecords(documentText)
        .filter(({ text }) => !isNarratedCvEditorialSegment(text))
        .flatMap((record) => getClaimsFromSegment(record.text)
            .map((claim) => ({ ...claim, recordIndex: record.index })))
        .sort((left, right) => left.recordIndex - right.recordIndex || left.index - right.index);
    const state = { seen: false, value: '', evidence: '', recordIndex: -1 };
    claims.forEach((claim) => {
        state.seen = true;
        if (claim.kind === 'positive') {
            state.value = claim.value;
            state.evidence = claim.evidence;
            state.recordIndex = claim.recordIndex;
            return;
        }
        if (claim.kind === 'uncertain') {
            if (!state.value) state.evidence = '';
            return;
        }
        if (claim.anaphoric && (!state.value || claim.recordIndex !== state.recordIndex + 1)) return;
        if (!state.value || claim.generic || !claim.value || areEquivalent(claim.value, state.value)) {
            state.value = '';
            state.evidence = '';
            state.recordIndex = claim.recordIndex;
        }
...
const getNarratedCvExplicitLocationState = (documentText = '') =>
    resolveNarratedCvScalarClaimState(documentText, getNarratedCvLocationClaimsFromSegment);

const getNarratedCvExplicitLocationFromSegment = (segment = '') =>
    getNarratedCvExplicitLocationState(segment).value;

const getNarratedCvExplicitLocation = (documentText = '') =>
    getNarratedCvExplicitLocationState(documentText).value;

const getNarratedCvExplicitPermitState = (documentText = '') => {
    const claims = getNarratedCvSegmentRecords(documentText)
        .filter(({ text }) => !isNarratedCvEditorialSegment(text))
        .flatMap((record) => getNarratedCvPermitClaimsFromSegment(record.text)
            .map((claim) => ({ ...claim, recordIndex: record.index })))
        .sort((left, right) => left.recordIndex - right.recordIndex || left.index - right.index);
    const activeClasses = new Map();
    let lastPositiveRecordIndex = -1;
    let seen = false;
    claims.forEach((claim) => {
        seen = true;
        const classKeys = [...new Set(getNarratedCvPermitClassKeys(claim.value))];
        if (claim.kind === 'positive') {
            classKeys.forEach((classKey) => activeClasses.set(classKey, {
                evidence: claim.evidence,
                recordIndex: claim.recordIndex,
            }));
            lastPositiveRecordIndex = claim.recordIndex;
            return;
        }
        if (claim.kind === 'uncertain') return;
        if (claim.anaphoric && (!activeClasses.size || claim.recordIndex !== lastPositiveRecordIndex + 1)) return;
        if (claim.kind === 'exclusive') {
            const inheritedEvidence = claim.anaphoric
                ? [...new Set([
                    ...[...activeClasses.values()].map((entry) => entry.evidence).filter(Boolean),
                    claim.evidence,
                ])].join(' ')
                : claim.evidence;
            activeClasses.clear();
        ...
        if (claim.generic || !classKeys.length) {
            activeClasses.clear();
            return;
        }
        classKeys.forEach((classKey) => activeClasses.delete(classKey));
    });
    const classKeys = [...activeClasses.keys()];
    const french = claims.some((claim) => /\bpermis\b/i.test(claim.value));
    const value = classKeys.length
        ? `${french ? 'Permis' : 'Driving licence'} ${classKeys.join(french ? ' et ' : ' and ')}`
        : '';
    const evidence = [...new Set([...activeClasses.values()].map((entry) => entry.evidence).filter(Boolean))].join(' ');
    return { seen, value, evidence, recordIndex: lastPositiveRecordIndex };
};

const getNarratedCvExplicitPermitFromSegment = (segment = '') =>
    getNarratedCvExplicitPermitState(segment).value;

const getNarratedCvExplicitPermit = (documentText = '') =>
    getNarratedCvExplicitPermitState(documentText).value;
...
const getNarratedCvSafeSourceProfile = (documentText = '') => {
    ...
    const profile = getNarratedCvSourceSegments(
        segmentRecords,
        /\b(?:je suis|j aime travailler|i am|i m|i enjoy working)\b/,
    ).find((segment) => !isNarratedCvWorkSegment(segment)
        && !isNarratedCvUndatedWorkSegment(segment)
        && !isNarratedCvSummaryWorkLikeSegment(segment)
        && !isNarratedCvLanguageStatement(segment)
        && !/\b(?:mon nom|je m appelle|my name)\b/i.test(segment)
        && getNarratedCvSummaryClauses(segment).every((clause) => {
            if (!isNarratedCvSummaryQualitySource(clause)) return true;
            const cleanedClause = cleanNarratedCvSummaryQualityEvidence(clause);
            const normalizedClause = stripAccents(normalizeText(clause).toLowerCase()).replace(/[’']/g, ' ');
            const normalizedCleanedClause = stripAccents(normalizeText(cleanedClause).toLowerCase()).replace(/[’']/g, ' ');
            return normalizedClause === normalizedCleanedClause
                || /\b(?:a\s+l\s+aise|comfortable)\b[\s\S]{0,24}\b(?:avec|with)\b/.test(normalizedClause);
        })
        && isSafeExtractedCvSummary(segment)
        && hasNarratedCvClauseGrounding(segment, documentText)
        && hasNarratedCvSummaryTokensGrounded(segment, documentText));
...
const getNarratedCvSummaryClauses = (value = '') => getNarratedCvOutputClauses(value)
    .flatMap((clause) => clause
        .split(/\s*;\s*|\s*[\/—–]\s*(?=(?:je|j[’']|i|mon|ma|mes|my|utilis(?:e|ant)|ma[iî]trise|sais|can|know|use|using|master|outils?|tools?|logiciels?|software)\b)|,\s*(?=(?:je|j[’']|i|mon|ma|mes|my|utilis(?:e|ant)|ma[iî]trise|sais|can|know|use|using|master|outils?|tools?|logiciels?|software)\b)|\s+(?:et|mais|and|but|puis|tout\s+en|then|while)\s+(?=(?:je|j[’']|i|mon|ma|mes|my|utilis(?:e|ant)|ma[iî]trise|sais|can|know|use|using|master|outils?|tools?|logiciels?|software)\b)/iu)
        .map((part) => part.trim())
        .filter(Boolean));

const cleanNarratedCvSummaryQualityEvidence = (value = '') => {
    const source = normalizeText(value);
    const normalizedSource = stripAccents(source.toLowerCase()).replace(/[’']/g, ' ');
    const statusRelation = normalizedSource.match(/\b(?:je\s+suis|j\s+etais|i\s+(?:am|m|was))\s+(?:(?:une?|la|le|les|an?|the)\s+)?(?:abonne\w*|adherent\w*|client\w*|customer|patron|participant\w*|utilisat\w*|user|etudiant\w*|eleve\w*|membre|member|subscriber|student|stagiaire|intern|benevole|volunteer|volontaire|fan|receptionniste|receptionist|employe\w*|employee|salarie\w*)\b[\s\S]*?\s+(?:a|au|aux|d|de|du|des|dans|pour|chez|at|of|in|for|with)\s+/);
    if (statusRelation) {
        const relationTail = statusRelation[0].match(/\s+(?:a|au|aux|d|de|du|des|dans|pour|chez|at|of|in|for|with)\s+$/)?.[0] || '';
        const relationIndex = statusRelation.index + statusRelation[0].length - relationTail.length;
        return normalizeText(source.slice(0, relationIndex));
    }
    const elidedRelation = source.match(/\s+d[’']/iu);
    if (elidedRelation) return normalizeText(source.slice(0, elidedRelation.index));
    const relationPattern = /\s+(à|au|aux|de|d[’']|du|des|of|pour|for|chez|at|avec|with|par|by|dans|in|sur|on)\s+/giu;
    for (const relation of source.matchAll(relationPattern)) {
        const relationName = stripAccents(relation[1].toLowerCase()).replace(/[’']/g, ' ');
        const relationSuffix = stripAccents(source.slice((relation.index || 0) + relation[0].length).toLowerCase())
            .replace(/[’']/g, ' ');
        if ((relationName === 'a' && /^l\s+aise\b/.test(relationSuffix))
            || (relationName === 'at' && /^ease\b/.test(relationSuffix))) {
            continue;
        }
        return normalizeText(source.slice(0, relation.index));
    }
    return source;
};

const isNarratedCvSummaryWorkLikeSegment = (value = '') => {
    if (isNarratedCvWorkSegment(value) || isNarratedCvUndatedWorkSegment(value)) return true;
    const source = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    const original = normalizeText(value);
    const copularWorkForNamedOrganization = /\b(?:[Jj]e\s+suis|[Jj][’']étais|[Ii]\s+am|[Ii]\s+was)\b[\s\S]{0,100}\b(?:pour|for)\s+(?:(?:l[’']|la|le|les|un|une|the)\s+)?\p{Lu}[\p{L}\p{M}'’.-]*/u.test(original);
    return copularWorkForNamedOrganization
        || /\b(?:je suis|j etais|i am|i was)\b[\s\S]{0,100}\b(?:chez|au sein de|a l hotel|a l entreprise|dans l entreprise|at (?:the )?(?:hotel|company|business|firm|agency|shop|store|restaurant)|with (?:the )?(?:company|business|firm|agency))\b/.test(source);
};

const isNarratedCvSummaryStatusSource = (value = '') => {
    const source = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    return /^(?:je\s+suis|j\s+etais|i\s+(?:am|m|was))\s+(?:(?:une?|la|le|les|an?|the)\s+)?(?:abonne\w*|adherent\w*|client\w*|customer|patron|participant\w*|utilisat\w*|user|etudiant\w*|eleve\w*|membre|member|subscriber|student|stagiaire|intern|benevole|volunteer|volontaire|fan|receptionniste|receptionist|employe\w*|employee|salarie\w*)\b/.test(source);
};

const isNarratedCvSummaryQualitySource = (value = '') => {
    const source = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    return /^(?:(?:je|j)\s+(?:(?:ne|n)\s+)?suis|j\s+(?:(?:ne|n)\s+)?aime\s+travailler|(?:mes\s+)?qualites?\s*:|i\s+(?:am|m)(?:\s+not)?|i\s+(?:do not |don t )?enjoy\s+working|my qualities\s*:)/.test(source)
        && !isNarratedCvSummaryWorkLikeSegment(value)
        && !isNarratedCvSummaryStatusSource(value)
        && !isNarratedCvLanguageStatement(value)
        && !/\b(?:mon nom|je m appelle|my name)\b/.test(source);
};

const isNarratedCvSummarySkillSource = (value = '') => {
    const source = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    return /^(?:utilis\w*|maitris\w*|sais|use|using|master\w*|can|know)\b/.test(source)
        || /\b(?:(?:je|j)\s+(?:(?:ne|n)\s+)?(?:sais|maitrise|utilise)|mes competences|outils?|savoir faire|i\s+(?:(?:do not|don t|never)\s+)?(?:can|know|use|master)|i\s+(?:cannot|can t)\b|my skills?|tools?)\b/.test(source);
};

const getNarratedCvSummarySourceRecords = (documentText = '') => {
    const documentLanguage = detectCvDocumentLanguage({}, documentText);
    const records = getNarratedCvSegmentRecords(documentText)
        .filter(({ text }) => !isNarratedCvEditorialSegment(text) && !isNarratedCvLanguageStatement(text));
    const sourceRecords = [];
    records.forEach(({ text, index }) => {
        getNarratedCvSummaryClauses(text).forEach((clause, clauseIndex) => {
            const typedRecords = [];
            if (isNarratedCvSummaryQualitySource(clause)) {
                const qualityEvidence = cleanNarratedCvSummaryQualityEvidence(clause);
                if (qualityEvidence) {
                    typedRecords.push({
                        id: `quality-${index}-${clauseIndex}`,
                        kind: 'quality',
                        text: qualityEvidence,
                        segmentIndex: index,
                    });
                    const normalizedClause = stripAccents(normalizeText(clause).toLowerCase()).replace(/[’']/g, ' ');
                    const normalizedEvidence = stripAccents(normalizeText(qualityEvidence).toLowerCase()).replace(/[’']/g, ' ');
                    if (normalizedClause !== normalizedEvidence) {
                        typedRecords.push({
                            id: `quality-full-${index}-${clauseIndex}`,
                            kind: 'quality-full',
                            text: clause,
                            relationTail: clause.slice(qualityEvidence.length),
                            segmentIndex: index,
                        });
                    }
                }
            }
            if (isNarratedCvSummarySkillSource(clause)) {
                typedRecords.push({
                    id: `skill-${index}-${clauseIndex}`,
                    kind: 'skill',
                    text: clause,
                    segmentIndex: index,
                });
            }
            sourceRecords.push(...typedRecords);
        });
        const normalizedRecord = stripAccents(normalizeText(text).toLowerCase()).replace(/[’']/g, ' ');
        if (/^(?:(?:enfin|finalement|en\s+fait|actually|ultimately)\s*,?\s*|correction\s*:\s*)?(?:(?:apres\s+reflexion|on\s+reflection)\s*,?\s*)?(?:non|no|pas\s+du\s+tout|plus\s+du\s+tout|absolument\s+pas|not\s+at\s+all|absolutely\s+not|plus\s+maintenant|not\s+anymore|no\s+longer|je\s+ne\s+le\s+suis\s+(?:pas|plus)(?:\s+du\s+tout)?|je\s+ne\s+pense\s+plus\s+que\s+ce\s+soit\s+vrai|ce\s+n\s+est\s+(?:vraiment\s+)?(?:desormais\s+)?plus\s+(?:le\s+cas|vrai)|i\s+am\s+not|i\s+no\s+longer\s+think\s+(?:that\s+)?(?:it|this|that)\s+is\s+true|that(?:\s+s|\s+is)\s+(?:no\s+longer\s+true|not\s+true\s+anymore))\s*[.!]?$/.test(normalizedRecord)) {
            const previousQuality = [...sourceRecords].reverse().find((record) =>
                record.kind === 'quality' && record.segmentIndex === index - 1
            );
            if (previousQuality) {
                sourceRecords.push({
                    id: `quality-retraction-${index}`,
                    kind: 'quality',
                    text: `pas ${previousQuality.text}`,
                    segmentIndex: index,
                });
            }
        }
    });
    const headlineRecords = records
        .map(({ text, index }) => ({
            id: `headline-${index}`,
            kind: 'headline',
            text: getExplicitCvHeadline(text, '', documentLanguage),
        }))
        .filter(({ text }) => Boolean(text));
    return [...sourceRecords, ...headlineRecords];
};

const normalizeNarratedCvSummaryToken = (value = '') => {
    const rawToken = stripAccents(normalizeText(value).toLowerCase()).replace(/^\.+|\.+$/g, '');
    if (/^(?:sourire|sourires|souriant|souriante|souriants|souriantes|smile|smiles|smiling)$/.test(rawToken)) {
        return 'cvsmile';
    }
    if (/^(?:aise|aisance|comfortable|comfort)$/.test(rawToken)) return 'cvcomfort';
    return normalizeNarratedCvSkillToken(rawToken);
};

const NARRATED_CV_SUMMARY_STOP_TOKENS = new Set([
    'suis', 'etre', 'etant', 'am', 'is', 'are', 'be', 'being', 'been',
]);

const isNarratedCvSummarySkillClause = (value = '') => {
    const source = stripAccents(normalizeText(value).toLowerCase()).replace(/[’']/g, ' ');
    return /^(?:utilis\w*|maitris\w*|sais|use|using|master\w*|can|know)\b/.test(source)
        || /\b(?:(?:je|j)\s+(?:sais|maitrise|utilise)|i\s+(?:can|know|use|master))\b/.test(source);
};

const getNarratedCvOrderedRelationTokens = (value = '') => stripAccents(normalizeText(value).toLowerCase())
    .replace(/[’']/g, ' ')
    .replace(/[^a-z0-9+#.]+/g, ' ')
    .trim()
    .split(/\s+/)
...
const hasNarratedCvOrderedTokenSequence = (sourceTokens = [], outputTokens = []) => {
    if (!sourceTokens.length || sourceTokens.length > outputTokens.length) return false;
    return outputTokens.some((_, index) => sourceTokens.every((token, offset) =>
        narratedCvTokensMatch(token, outputTokens[index + offset])
    ));
};

const hasNarratedCvSummaryClauseTokensGrounded = (summaryClause = '', sourceRecords = []) => {
    const summaryRecords = getNarratedCvSkillTokenRecords(summaryClause, { checkAffirmation: false })
        .filter(({ token }) => !NARRATED_CV_SUMMARY_STOP_TOKENS.has(token));
    if (!summaryRecords.length) return false;
    const summaryTokens = new Set(summaryRecords.map(({ token }) => normalizeNarratedCvSummaryToken(token)));
    const allowedKind = isNarratedCvSummarySkillClause(summaryClause) ? 'skill' : 'quality';
    const allowedRecords = sourceRecords.filter(({ kind }) => kind === allowedKind);
    const summaryRelationTokens = getNarratedCvOrderedRelationTokens(summaryClause);
    const eligibleFullQualityRecords = sourceRecords.filter(({ kind, text, relationTail }) => {
        if (kind !== 'quality-full' || allowedKind !== 'quality') return false;
        const fullTokens = getNarratedCvSkillTokenRecords(text, { checkAffirmation: false })
            .map(({ token }) => normalizeNarratedCvSummaryToken(token))
            .filter((token) => !NARRATED_CV_SUMMARY_STOP_TOKENS.has(token));
        const orderedFullTokensGrounded = fullTokens.length > 0
            && hasNarratedCvOrderedTokenSequence(
                fullTokens,
                summaryRecords.map(({ token }) => normalizeNarratedCvSummaryToken(token)),
            );
        const relationTokens = getNarratedCvOrderedRelationTokens(relationTail);
        return orderedFullTokensGrounded
            && relationTokens.length > 0
            && hasNarratedCvOrderedTokenSequence(relationTokens, summaryRelationTokens);
    });
    const eligibleHeadlineRecords = sourceRecords.filter(({ kind, text }) => {
        if (kind !== 'headline' || allowedKind !== 'quality') return false;
        const headlineTokens = getNarratedCvSkillTokenRecords(text, { checkAffirmation: false })
            .map(({ token }) => normalizeNarratedCvSummaryToken(token))
            .filter((token) => !NARRATED_CV_SUMMARY_STOP_TOKENS.has(token));
        return headlineTokens.length > 0 && headlineTokens.every((token) => summaryTokens.has(token));
    });
    const sourceTokenRecords = [...allowedRecords, ...eligibleFullQualityRecords, ...eligibleHeadlineRecords]
        .flatMap(({ id, kind, text }) => getNarratedCvSkillTokenRecords(text, {
            checkAffirmation: kind !== 'headline' && kind !== 'quality-full',
        }).map((record) => ({ ...record, sourceId: id, kind })))
        .filter(({ token }) => !NARRATED_CV_SUMMARY_STOP_TOKENS.has(token));
    const sourceTokenCounts = sourceTokenRecords.reduce((counts, { token }) => {
        const normalizedToken = normalizeNarratedCvSummaryToken(token);
        counts.set(normalizedToken, (counts.get(normalizedToken) || 0) + 1);
        return counts;
    }, new Map());
    const summaryTokenCounts = summaryRecords.reduce((counts, { token }) => {
        const normalizedToken = normalizeNarratedCvSummaryToken(token);
        counts.set(normalizedToken, (counts.get(normalizedToken) || 0) + 1);
        return counts;
    }, new Map());
    if ([...summaryTokenCounts].some(([token, count]) => count > (sourceTokenCounts.get(token) || 0))) {
        return false;
    }
    return summaryRecords.every(({ token }) => {
        const normalizedToken = normalizeNarratedCvSummaryToken(token);
        const matchingSourceRecords = sourceTokenRecords.filter((sourceRecord) =>
            normalizeNarratedCvSummaryToken(sourceRecord.token) === normalizedToken
        );
        const categoricalMatches = matchingSourceRecords.filter(({ kind }) => kind === allowedKind);
        if (categoricalMatches.length) return categoricalMatches.at(-1).affirmed;
        return matchingSourceRecords.some(({ kind, affirmed }) =>
            (kind === 'headline' || kind === 'quality-full') && affirmed
        );
    });
};

const hasNarratedCvSummaryTokensGrounded = (summary = '', documentText = '') => {
    const clauses = getNarratedCvSummaryClauses(summary);
    if (!clauses.length) return false;
    const sourceRecords = getNarratedCvSummarySourceRecords(documentText);
    return clauses.every((clause) => hasNarratedCvSummaryClauseTokensGrounded(clause, sourceRecords));
};

const hasNarratedCvSummaryClauseGrounding = (summary = '', documentText = '') => {
    const clauses = getNarratedCvSummaryClauses(summary);
    return clauses.length > 0 && clauses.every((clause) =>
        hasNarratedCvSummaryTokensGrounded(clause, documentText)
        && hasNarratedCvSensitiveClaimGrounding(clause, documentText)
    );
};
...
const reconcileNarratedCvAssistantResult = (result, documentText = '') => {
    ...
    const explicitLocationState = getNarratedCvExplicitLocationState(documentText);
    ...
    const summaryNeedsGroundedFallback = !modelSummary
        || !hasNarratedCvSummaryClauseGrounding(modelSummary, documentText)
        || !hasNarratedCvSummaryTokensGrounded(modelSummary, documentText);
...
const hasCompleteNarratedCvExtraction = (value, documentText = '', diagnostics = null) => {
    ...
    const rawWorkRecords = getNarratedCvWorkRecords(segmentRecords);
    ...
    const educationSegments = segmentRecords
        .flatMap(({ text }) => normalizeText(text)
            .split(/\s+(?:et|and)\s+(?=(?:je|j[’']|i)\b)/iu)
            .map((segment) => segment.trim())
            .filter(Boolean))
        .filter((segment) => {
        const normalized = stripAccents(normalizeText(segment).toLowerCase()).replace(/[’']/g, ' ');
        const permitClaims = getNarratedCvPermitClaimsFromSegment(segment);
        const hasUnambiguousEducationSignal = /\b(?:formation|diplome|certificat|certification|cap|bts|bac|master|ecole|lycee|cfa|universite|training|degree|certificate|school|university|college)\b/.test(normalized);
        return /\b(?:formation|diplome|certificat|certification|cap|bts|bac|licence|master|ecole|cfa|universite|training|degree|certificate|school|university|college)\b/.test(normalized)
            ...
            && (!permitClaims.length || hasUnambiguousEducationSignal);
        });
    const formalEducationSegments = educationSegments.filter((segment) => {
        const normalized = stripAccents(normalizeText(segment).toLowerCase()).replace(/[’']/g, ' ');
        const explicitFormalQualification = /\b(?:certificat d aptitude professionnelle|diplome|cap|bts|bac|licence|master|degree|diploma|graduated)\b/.test(normalized);
        const educationInstitution = /\b(?:ecole|lycee|cfa|universite|school|university|college)\b/.test(normalized);
        const explicitShortCertification = /\b(?:certification|certificat|attestation|formation courte|premiers secours|first aid|certificate|short course)\b/.test(normalized);
        return explicitFormalQualification || (educationInstitution && !explicitShortCertification);
    });
    const certificationSegments = educationSegments.filter((segment) => !formalEducationSegments.includes(segment));
    const explicitSkillSegments = getNarratedCvSkillSourceParagraphs(
        segmentRecords,
    ...
    const qualitySegments = getNarratedCvSourceSegments(
        segmentRecords,
        /\b(?:je suis|j aime travailler|(?:mes\s+)?qualites?|i am|i m|i enjoy working|my qualities)\b/,
    ).filter((segment) => !isNarratedCvWorkSegment(segment)
        && !isNarratedCvLanguageStatement(segment)
        && !/\b(?:mon nom|je m appelle|my name)\b/i.test(segment));
    const skillSourceSegments = [
        ...workRecords.map((record) => ({ text: record.context, kind: 'work' })),
        ...segmentRecords
            .filter(({ text }) => hasNarratedCvFollowUpReference(text) && !isNarratedCvEditorialSegment(text))
            .map(({ text }) => ({ text, kind: 'work' })),
        ...explicitSkillSegments.map((text) => ({ text, kind: 'explicit' })),
        ...qualitySegments.map((text) => ({ text, kind: 'quality' })),
    ].filter(Boolean);
    const allSkillAssertionSegments = segmentRecords
        .filter(({ text }) => !isNarratedCvEditorialSegment(text) && !isNarratedCvLanguageStatement(text))
        .map(({ text }) => ({ text, kind: 'generic' }));
    const activitySegments = segmentRecords
        .filter(({ text }) => isNarratedCvActivityStatement(text) && !isNarratedCvEditorialSegment(text))
        .map(({ text }) => getNarratedCvActivityContent(text))
        .filter(Boolean);
    ...
    const explicitLocationState = getNarratedCvExplicitLocationState(source);
    const locationSegments = explicitLocationState.value
        ? [explicitLocationState.evidence || explicitLocationState.value]
        : [];
    const explicitPermitState = getNarratedCvExplicitPermitState(source);
    const permitSegments = explicitPermitState.value
    ...
    const narratedValidationChecks = {
        structuredExtraction: hasStructuredCvExtraction(extracted),
        noInstructionLeak: !containsInstructionLeak,
        ...
        headlineGrounded,
        summaryGrounded,
        summaryClausesGrounded,
        summaryTokensGrounded,
        summaryNumbersGrounded,
        skillsGrounded,
        activitiesGrounded,
        projectsGrounded,
        educationGrounded,
        educationClausesGrounded,
        noPermitAsEducation,
        languagesGrounded,
    };
    const failedNarratedValidationChecks = Object.entries(narratedValidationChecks)
    ...
    if (episodeAssignment) {
        ...
        for (const record of segmentRecords) {
            ...
            const explicitlyCurrent = /\b(?:emploi|poste|travail) actuel|\bcurrent (?:job|role|work)\b/.test(normalizedRecord);
...
module.exports = async (request, response) => {
    ...
    if (isDedicatedCvRequest) {
        ...
        const legacyDocumentText = !suppliedDocumentText
            ...
            ? rawInstruction
            : '';
        const rawDocumentText = suppliedDocumentText || legacyDocumentText;
        const documentText = requestedSourceKind === 'narrative'
            ? normalizeNarratedCvSourceText(rawDocumentText)
            : rawDocumentText;
        ...
        const deterministicHeadline = getExplicitCvHeadline(instruction, cv.headline, cv.documentLanguage);
        const finalizedFallback = deterministicHeadline
        ...
        return json(response, 200, {
            ...
            cv: requestedSourceKind === 'narrative'
                ? reconcileNarratedCvAssistantResult(finalizedFallback, documentText)
                : finalizedFallback,
        });
    }
